...
let initWithUniqueDisplays x ~changeSeed ~flipOffset ~reconfigureSeed ~acceleratorSeed self = msg_send ~self ~cmd:(selector "initWithUniqueDisplays:changeSeed:flipOffset:reconfigureSeed:acceleratorSeed:") ~typ:(id @-> uint @-> double @-> ullong @-> ullong @-> returning (id)) x changeSeed flipOffset reconfigureSeed acceleratorSeed
