...
let sendCarbonUpdateHICommandStatusEvent x ~withMenuRef ~andMenuItemIndex self = msg_send ~self ~cmd:(selector "sendCarbonUpdateHICommandStatusEvent:withMenuRef:andMenuItemIndex:") ~typ:(uint @-> ptr void @-> ushort @-> returning (bool)) x withMenuRef andMenuItemIndex
