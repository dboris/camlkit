...
let sizeSnapRectForFrame x ~snappedEdges self = msg_send_stret ~self ~cmd:(selector "sizeSnapRectForFrame:snappedEdges:") ~typ:(CGRect.t @-> ullong @-> returning (CGRect.t)) ~return_type:CGRect.t x snappedEdges
