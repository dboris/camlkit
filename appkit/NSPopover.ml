...
let _class_ = get_class "NSPopover"
...
let anchorSize self = msg_send ~self ~cmd:(selector "anchorSize") ~typ:(returning (CGSize.t))
...
let setPositioningOptions x self = msg_send ~self ~cmd:(selector "setPositioningOptions:") ~typ:(ullong @-> returning (void)) x
