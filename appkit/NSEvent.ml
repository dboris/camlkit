...
module C = struct
  ...
  let eventWithCGEvent x self = msg_send ~self ~cmd:(selector "eventWithCGEvent:") ~typ:(ptr void @-> returning (id)) x
...
let _CGEvent self = msg_send ~self ~cmd:(selector "CGEvent") ~typ:(returning (ptr void))
