...
module Class = struct
  let modifierForEffect x self = msg_send ~self ~cmd:(selector "modifierForEffect:") ~typ:(llong @-> returning (id)) x
end
...
let initWithSystemEffect x self = msg_send ~self ~cmd:(selector "initWithSystemEffect:") ~typ:(llong @-> returning (id)) x
