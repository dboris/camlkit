...
let _class_ = get_class "NSTableHeaderCell"
...
let drawSortIndicatorWithFrame x ~inView ~ascending ~priority self = msg_send ~self ~cmd:(selector "drawSortIndicatorWithFrame:inView:ascending:priority:") ~typ:(CGRect.t @-> id @-> bool @-> llong @-> returning (void)) x inView ascending priority
...
let sortIndicatorRectForBounds x self = msg_send ~self ~cmd:(selector "sortIndicatorRectForBounds:") ~typ:(CGRect.t @-> returning (CGRect.t)) x