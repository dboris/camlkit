...
let flushWithOptions x self = msg_send ~self ~cmd:(selector "flushWithOptions:") ~typ:(ullong @-> returning (void)) x
...
let orderSurface x ~relativeTo self = msg_send ~self ~cmd:(selector "orderSurface:relativeTo:") ~typ:(llong @-> id @-> returning (void)) x relativeTo
