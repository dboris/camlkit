...
let _class_ = get_class "NSCollectionViewChildProxy"

module Class = struct
  let proxyForItemAtIndex x ~collectionView self = msg_send ~self ~cmd:(selector "proxyForItemAtIndex:collectionView:") ~typ:(ullong @-> id @-> returning (id)) x collectionView
end
...
let initWithIndex x ~collectionView self = msg_send ~self ~cmd:(selector "initWithIndex:collectionView:") ~typ:(ullong @-> id @-> returning (id)) x collectionView
