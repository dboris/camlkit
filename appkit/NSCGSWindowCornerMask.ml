...
let initWithImage x ~scale ~center ~hasTrivialEdges ~clipsWindowContents ~definesShadowShape self = msg_send ~self ~cmd:(selector "initWithImage:scale:center:hasTrivialEdges:clipsWindowContents:definesShadowShape:") ~typ:(id @-> double @-> CGRect.t @-> bool @-> bool @-> bool @-> returning (id)) x scale center hasTrivialEdges clipsWindowContents definesShadowShape
