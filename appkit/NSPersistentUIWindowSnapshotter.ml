...
let captureAndWriteSnapshotForWindowNumber x ~forWindowID ~waitUntilDone self = msg_send ~self ~cmd:(selector "captureAndWriteSnapshotForWindowNumber:forWindowID:waitUntilDone:") ~typ:(llong @-> uint @-> bool @-> returning (void)) x forWindowID waitUntilDone
...
let enqueueWindowForSnapshotting x ~forWindowNumber self = msg_send ~self ~cmd:(selector "enqueueWindowForSnapshotting:forWindowNumber:") ~typ:(uint @-> llong @-> returning (void)) x forWindowNumber
