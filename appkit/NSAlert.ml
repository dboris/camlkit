...
let buildAlertStyle' x ~title ~message ~first ~second ~third ~oldStyle ~args self = msg_send ~self ~cmd:(selector "buildAlertStyle:title:message:first:second:third:oldStyle:args:") ~typ:(ullong @-> id @-> id @-> id @-> id @-> id @-> bool @-> ptr void @-> returning (id)) (ULLong.of_int x) title message first second third oldStyle args
