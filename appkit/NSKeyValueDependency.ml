...
let initWithContext' x ~invalidationCount self = msg_send ~self ~cmd:(selector "initWithContext:invalidationCount:") ~typ:(id @-> ullong @-> returning (id)) x invalidationCount
...
let setInvalidationCount x self = msg_send ~self ~cmd:(selector "setInvalidationCount:") ~typ:(ullong @-> returning (void)) x