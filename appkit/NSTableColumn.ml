...
let _class_ = get_class "NSTableColumn"
...
let dataCellForRow x self = msg_send ~self ~cmd:(selector "dataCellForRow:") ~typ:(llong @-> returning (id)) x
...
let setResizingMask x self = msg_send ~self ~cmd:(selector "setResizingMask:") ~typ:(ullong @-> returning (void)) x
