...
let initWithRow x ~tableColumn ~realElement self = msg_send ~self ~cmd:(selector "initWithRow:tableColumn:realElement:") ~typ:(llong @-> id @-> id @-> returning (id)) x tableColumn realElement
let isEqual x self = msg_send ~self ~cmd:(selector "isEqual:") ~typ:(id @-> returning (bool)) x
let realElementRect self = msg_send ~self ~cmd:(selector "realElementRect") ~typ:(returning (CGRect.t))