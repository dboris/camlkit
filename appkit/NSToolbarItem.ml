...
let _class_ = get_class "NSToolbarItem"
...
let setVisibilityPriority x self = msg_send ~self ~cmd:(selector "setVisibilityPriority:") ~typ:(llong @-> returning (void)) x
...
let usesMenuFormRepresentationInDisplayMode x self = msg_send ~self ~cmd:(selector "usesMenuFormRepresentationInDisplayMode:") ~typ:(ullong @-> returning (bool)) x
