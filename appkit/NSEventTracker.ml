...
let _class_ = get_class "NSEventTracker"
...
let setDeviceID x self = msg_send ~self ~cmd:(selector "setDeviceID:") ~typ:(ullong @-> returning (void)) x
let setModifierFlags x self = msg_send ~self ~cmd:(selector "setModifierFlags:") ~typ:(ullong @-> returning (void)) x
let setPhase x self = msg_send ~self ~cmd:(selector "setPhase:") ~typ:(ullong @-> returning (void)) x
