...
module Class = struct
  let partWithCode x ~parent self = msg_send ~self ~cmd:(selector "partWithCode:parent:") ~typ:(ullong @-> id @-> returning (id)) x parent
end
...
let initWithPartCode x ~parent self = msg_send ~self ~cmd:(selector "initWithPartCode:parent:") ~typ:(ullong @-> id @-> returning (id)) x parent
