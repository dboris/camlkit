...
let setCandidates x ~andCorrectionPanelType self = msg_send ~self ~cmd:(selector "setCandidates:andCorrectionPanelType:") ~typ:(id @-> llong @-> returning (void)) x andCorrectionPanelType
...
let textFieldAtIndex x self = msg_send ~self ~cmd:(selector "textFieldAtIndex:") ~typ:(ullong @-> returning (id)) x