...
let _class_ = get_class "NSLocalSavePanel"
...
module Class = struct
  ...
  let kvoKeysForType x self = msg_send ~self ~cmd:(selector "kvoKeysForType:") ~typ:(llong @-> returning (id)) x
...
let overwriteAlertDidEnd x ~returnCode ~contextInfo self = msg_send ~self ~cmd:(selector "overwriteAlertDidEnd:returnCode:contextInfo:") ~typ:(id @-> llong @-> id @-> returning (void)) x returnCode contextInfo
...
let seamlessOpener x ~sourceFrameOnScreenForPreviewItem self = msg_send_stret ~self ~cmd:(selector "seamlessOpener:sourceFrameOnScreenForPreviewItem:") ~typ:(id @-> id @-> returning (CGRect.t)) ~return_type:CGRect.t x sourceFrameOnScreenForPreviewItem
