...
include NSScrollingBehavior

let _class_ = get_class "NSScrollingBehaviorConcurrentVBL"
...
let scrollStateEvent x ~sender self = msg_send ~self ~cmd:(selector "scrollStateEvent:sender:") ~typ:(llong @-> id @-> returning (void)) x sender
...
let setGestureToken x self = msg_send ~self ~cmd:(selector "setGestureToken:") ~typ:(ullong @-> returning (void)) x
