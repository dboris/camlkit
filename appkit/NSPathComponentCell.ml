...
let _class_ = get_class "NSPathComponentCell"
...
let titleRectForBounds' x ~imageRect self = msg_send_stret ~self ~cmd:(selector "titleRectForBounds:imageRect:") ~typ:(CGRect.t @-> CGRect.t @-> returning (CGRect.t)) ~return_type:CGRect.t x imageRect