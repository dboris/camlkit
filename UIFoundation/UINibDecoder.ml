...
let self = get_class "UINibDecoder"
...
let uniqueIDForCurrentlyDecodingObject self = msg_send ~self ~cmd:(selector "uniqueIDForCurrentlyDecodingObject") ~typ:(returning llong) |> LLong.to_int
