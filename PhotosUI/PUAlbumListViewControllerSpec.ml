...
let collageImageSize self = msg_send_stret ~self ~cmd:(selector "collageImageSize") ~typ:(returning CGSize.t) ~return_type:CGSize.t
...
let tileSizeForLayoutReferenceSize x ~safeAreaInsets ~stackViewStyle self = msg_send_stret ~self ~cmd:(selector "tileSizeForLayoutReferenceSize:safeAreaInsets:stackViewStyle:") ~typ:(CGSize.t @-> UIEdgeInsets.t @-> ullong @-> returning CGSize.t) ~return_type:CGSize.t x safeAreaInsets (ULLong.of_int stackViewStyle)
