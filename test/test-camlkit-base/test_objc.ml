...
let test_object_description () =
  let desc =
    get_class "NSObject" |> alloc |> NSObject.init |> NSString.description
...
let test_add_method () =
  let added =
    Class.add_method ~self:(get_class "NSObject") ~cmd:(selector "addOneTo:")
...
let test_define_class_with_methods () =
  ...
  and enc = Objc_type.(encode_method ~args:[ int ] int) in
  let methods = [ method_spec ~cmd ~typ imp ~enc ] and x = 5 in
  let c = Class.define name ~methods in
  let defined = not (is_null c)
  and y = Objc.msg_send ~self:(_new_ c) ~cmd ~typ x in
  A.check A.bool "class ptr not null" defined true;
...
let dealloc_spec called_flag =
  ...
  in
  method_spec ~cmd:(selector "dealloc") ~typ:(returning void) imp
    ~enc:Objc_type.(encode_method ~args:[] void)

let test_gc_autorelease () =
  let dealloc_called = ref false in
  let name = "MyClass3" and methods = [ dealloc_spec dealloc_called ] in
  let c = Class.define name ~methods in
...
let test_add_protocol () =
  ...
  in
  let c = Class.define ~protocols ~methods name and proto = List.hd protocols in
  A.check A.bool "class ptr not null" (is_null c) false;
  A.check A.bool "class conforms to protocol"
    (Class.conforms_to_protocol c proto)
    true
...
let test_add_ivar ~name x () =
  ...
  and methods =
    [
      Property.getter "myVar" ~typ:int ~enc:Objc_type.(encode_value int);
      Property.setter "myVar" ~typ:int
        ~enc:Objc_type.(encode_method ~args:[ int ] void);
    ]
  in
  let o = _new_ (Class.define name ~ivars ~methods) in
  Objc.msg_send ~self:o ~cmd:(selector "setMyVar:")
    ~typ:(int @-> returning void)
    x;
  let v = Objc.msg_send ~self:o ~cmd:(selector "myVar") ~typ:(returning int) in
  A.check A.int "set value and get same value" x v

let test_value_accessors ~name x () =
  let ivars = [ Ivar.define "myVar" Objc_type.int ]
  and methods = Property.accessor_methods "myVar" Objc_type.int in
  let self = _new_ (Class.define name ~ivars ~methods) in
  Objc.msg_send ~self ~cmd:(selector "setMyVar:")
    ~typ:(int @-> returning void)
    x;
  let v = Objc.msg_send ~self ~cmd:(selector "myVar") ~typ:(returning int) in
  A.check A.int "set value and get same value" x v
...
let test_value_property ~name x () =
  ...
  let v = self |> Property.get "myVar" Objc_type.int in
...
let test_object_property ~name x () =
  ...
  and methods = Property.accessor_methods "myVar" Objc_type.id in
  let self = _new_ (Class.define name ~ivars ~methods) in
  self |> Property.set "myVar" x Objc_type.id;
  let v = self |> Property.get "myVar" Objc_type.id in
  A.check A.string "set property value and get same value"
    (NSString._UTF8String x) (NSString._UTF8String v)

let test_object_accessors ~name x () =
  let ivars = [ Ivar.define "myVar" Objc_type.id ]
  and methods = Property.accessor_methods "myVar" Objc_type.id in
  let self = _new_ (Class.define name ~ivars ~methods) in
  Objc.msg_send ~self ~cmd:(selector "setMyVar:") ~typ:(id @-> returning void) x;
  let v = Objc.msg_send ~self ~cmd:(selector "myVar") ~typ:(returning id) in
  A.check A.string "set value and get same value" (NSString._UTF8String x)
...
let test_add_obj_ivar ~name x () =
  ...
  and methods =
    [
      Property.getter "myVar" ~typ:id ~enc:Objc_type.(encode_value id);
      Property.setter "myVar" ~typ:id
        ~enc:Objc_type.(encode_method ~args:[ id ] void);
    ]
  in
  let o = _new_ (Class.define name ~ivars ~methods) in
  Objc.msg_send ~self:o ~cmd:(selector "setMyVar:")
    ~typ:(id @-> returning void)
    x;
  let v = Objc.msg_send ~self:o ~cmd:(selector "myVar") ~typ:(returning id) in
  A.check A.string "set value and get same value" (NSString._UTF8String x)
    (NSString._UTF8String v)
...
let test_string_of_selector () =
  let expected = "myVar" and actual = string_of_selector (selector "myVar") in
...
let test_block () =
  ...
  let block =
    Block.make (* ~typ: (id @-> id @-> ullong @-> bool @-> returning void) *)
      ~args:Objc_type.[ id; ullong; bool ]
      ~return:Objc_type.void
      (fun _self obj idx _stop ->
        actual :=
          Printf.sprintf "%d: %s" (ULLong.to_int idx)
...
let test_msg_send_super () =
  ...
  let class_a =
    Class.define "ClassA"
      ~methods:
        [
          Method.define ~cmd:(selector "someMethod")
            ~args:Objc_type.[]
            ~return:Objc_type.void
  ...
  in
  let class_b =
    Class.define "ClassB" ~superclass:class_a
      ~methods:
        [
          Method.define ~cmd:(selector "someMethod")
            ~args:Objc_type.[]
...
let test_set_and_get_object_ivar ~name x () =
  ...
  o |> set_ivar ivar_name x ivar_t;
  let v = o |> get_ivar ivar_name ivar_t in
  A.check A.string "set ivar value and get same value" (NSString._UTF8String x)
    (NSString._UTF8String v)
...
let suite =
  [
    ...
    ( "set and get ivar directly",
      `Quick,
      test_set_and_get_ivar ~name:"MyClass14" 53 );
    ( "set and get object ivar directly",
      `Quick,
