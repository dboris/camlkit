...
module NSInvocation = struct
  ...
  *)
  let get_argument ~typ ~init ~at_index self =
    ...
    let () = self |> getArgument (to_voidp arg) ~atIndex:at_index in
    !@arg
...
let test_define_custom_class () =
  ...
  let module MyCustomClass = CamlProxy.Create (struct
    ...
    let method_signature_for_selector sel =
      ...
      else Objc_type.(encode_value unknown)
  ...
  let expected = 5 * 2
  and actual =
    Objc.msg_send ~self:obj ~cmd:(selector my_sel)
...
let test_define_CamlObjectProxy_class () =
  let module NSStringCamlProxy = CamlObjectProxy.Create (struct
    let class_name = "NSStringCamlProxy"
    let responds_to_selector = function "multByThree:" -> true | _ -> false
  ...
  let expected = x * 3
  and actual =
    Objc.msg_send ~self:obj ~cmd:(selector "multByThree:")
      ~typ:(int @-> returning int)
      x
  in
  A.check A.int "caml method called" expected actual;
  A.check A.string "target object method forwarded" str
    (NSString._UTF8String obj)

let test_nsarray_list_roundtrip () =
  let msg = [ "Hello"; "World" ] in
  let actual =
    msg |> List.map new_string |> NSArray.of_list |> NSArray.to_list
    |> List.map to_string
  in
  A.(check (list string)) "same list" msg actual

let test_nsdict_list_roundtrip () =
  let msg = [ ("Hello", "Hola"); ("World", "Mundo") ] in
  let actual =
    msg
    |> List.map (fun (k, v) -> (new_string k, new_string v))
    |> NSDictionary.of_list |> NSDictionary.to_list
    |> List.map (fun (k, v) -> (to_string k, to_string v))
  in
  A.(check (list (pair string string))) "same list" msg actual

let suite =
  [
    ("define custom class", `Quick, test_define_custom_class);
    ("define CamlObjectProxy class", `Quick, test_define_CamlObjectProxy_class);
    ("NSArray - List roundtrip", `Quick, test_nsarray_list_roundtrip);
    ("NSDictionary - List roundtrip", `Quick, test_nsdict_list_roundtrip);
  ]
