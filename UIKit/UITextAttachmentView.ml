...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uitextattachmentview?language=objc}UITextAttachmentView} *)

let self = get_class "UITextAttachmentView"
...
let drawWithFrame x ~inView ~characterIndex self = msg_send ~self ~cmd:(selector "drawWithFrame:inView:characterIndex:") ~typ:(CGRect.t @-> id @-> ullong @-> returning void) x inView (ULLong.of_int characterIndex)
