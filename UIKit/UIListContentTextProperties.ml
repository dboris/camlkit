...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uilistcontenttextproperties?language=objc}UIListContentTextProperties} *)

let self = get_class "UIListContentTextProperties"
...
let transform self = msg_send ~self ~cmd:(selector "transform") ~typ:(returning llong) |> LLong.to_int