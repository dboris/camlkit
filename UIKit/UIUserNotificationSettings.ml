...
let allowedUserNotificationTypes self = msg_send ~self ~cmd:(selector "allowedUserNotificationTypes") ~typ:(returning ullong) |> ULLong.to_int
...
let types self = msg_send ~self ~cmd:(selector "types") ~typ:(returning ullong) |> ULLong.to_int
