...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiactionsheet?language=objc}UIActionSheet} *)

let self = get_class "UIActionSheet"

let actionSheetStyle self = msg_send ~self ~cmd:(selector "actionSheetStyle") ~typ:(returning llong) |> LLong.to_int
...
let destructiveButtonIndex self = msg_send ~self ~cmd:(selector "destructiveButtonIndex") ~typ:(returning llong) |> LLong.to_int
