...
let contentRectForCell x ~forState self = msg_send ~self ~cmd:(selector "contentRectForCell:forState:") ~typ:(id @-> ullong @-> returning CGRect.t) x (ULLong.of_int forState)
let contentRectForCell' x ~forState ~rowWidth self = msg_send ~self ~cmd:(selector "contentRectForCell:forState:rowWidth:") ~typ:(id @-> ullong @-> double @-> returning CGRect.t) x (ULLong.of_int forState) rowWidth
