...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uisearchdisplaycontroller?language=objc}UISearchDisplayController} *)

let self = get_class "UISearchDisplayController"
...
let searchBar1 x ~selectedScopeButtonIndexDidChange self = msg_send ~self ~cmd:(selector "searchBar:selectedScopeButtonIndexDidChange:") ~typ:(id @-> llong @-> returning void) x (LLong.of_int selectedScopeButtonIndexDidChange)
...
let setNavigationBarSearchFieldSizing x self = msg_send ~self ~cmd:(selector "setNavigationBarSearchFieldSizing:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
