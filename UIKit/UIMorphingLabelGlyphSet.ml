...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uimorphinglabelglyphset?language=objc}UIMorphingLabelGlyphSet} *)

let self = get_class "UIMorphingLabelGlyphSet"

let attributedString self = msg_send ~self ~cmd:(selector "attributedString") ~typ:(returning id)
let boundingRectForGlyphsInRange x self = msg_send_stret ~self ~cmd:(selector "boundingRectForGlyphsInRange:") ~typ:(NSRange.t @-> returning CGRect.t) ~return_type:CGRect.t x
...
let lineOrigin self = msg_send_stret ~self ~cmd:(selector "lineOrigin") ~typ:(returning CGPoint.t) ~return_type:CGPoint.t
