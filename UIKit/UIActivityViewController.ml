...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiactivityviewcontroller?language=objc}UIActivityViewController} *)

let self = get_class "UIActivityViewController"
...
let excludedActivityCategories self = msg_send ~self ~cmd:(selector "excludedActivityCategories") ~typ:(returning llong) |> LLong.to_int
...
let sharingStyle self = msg_send ~self ~cmd:(selector "sharingStyle") ~typ:(returning llong) |> LLong.to_int
