...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiwebpdfviewhandler?language=objc}UIWebPDFViewHandler} *)
...
let scalesForContainerSize x self = msg_send ~self ~cmd:(selector "scalesForContainerSize:") ~typ:(CGSize.t @-> returning void) x
let scalesPageToFit self = msg_send ~self ~cmd:(selector "scalesPageToFit") ~typ:(returning bool)
let scrollToPageNumber x ~animate self = msg_send ~self ~cmd:(selector "scrollToPageNumber:animate:") ~typ:(llong @-> bool @-> returning void) (LLong.of_int x) animate
...
let updateBoundariesOfScrollView x ~withScales self = msg_send ~self ~cmd:(selector "updateBoundariesOfScrollView:withScales:") ~typ:(id @-> void @-> returning void) x withScales
