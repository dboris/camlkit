...
let contentInsetsReference self = msg_send ~self ~cmd:(selector "contentInsetsReference") ~typ:(returning llong) |> LLong.to_int
let copyWithZone x self = msg_send ~self ~cmd:(selector "copyWithZone:") ~typ:((ptr void) @-> returning id) x
let differencesFromConfiguration x self = msg_send ~self ~cmd:(selector "differencesFromConfiguration:") ~typ:(id @-> returning ullong) x |> ULLong.to_int
