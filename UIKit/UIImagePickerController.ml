...
let self = get_class "UIImagePickerController"
...
let cameraViewTransform self = msg_send ~self ~cmd:(selector "cameraViewTransform") ~typ:(returning CGAffineTransform.t)
...
let setCameraViewTransform x self = msg_send ~self ~cmd:(selector "setCameraViewTransform:") ~typ:(CGAffineTransform.t @-> returning void) x
