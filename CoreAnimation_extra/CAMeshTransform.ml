...
let faceAtIndex x self = msg_send_stret ~self ~cmd:(selector "faceAtIndex:") ~typ:(ullong @-> returning CAMeshFace.t) ~return_type:CAMeshFace.t (ULLong.of_int x)
...
let vertexAtIndex x self = msg_send_stret ~self ~cmd:(selector "vertexAtIndex:") ~typ:(ullong @-> returning CAMeshVertex.t) ~return_type:CAMeshVertex.t (ULLong.of_int x)
