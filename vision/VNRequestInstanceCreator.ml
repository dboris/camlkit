...
let newRequestInstanceOfClass x ~withCompletionHandler ~revision ~error self = msg_send ~self ~cmd:(selector "newRequestInstanceOfClass:withCompletionHandler:revision:error:") ~typ:(_Class @-> ptr void @-> ullong @-> ptr (id) @-> returning (id)) x withCompletionHandler (ULLong.of_int revision) error