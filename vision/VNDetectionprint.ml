...
module Class = struct
  let knownTensorKeysForRequestRevision x ~error self = msg_send ~self ~cmd:(selector "knownTensorKeysForRequestRevision:error:") ~typ:(ullong @-> ptr (id) @-> returning (id)) x error
...
let initWithTensorsDictionary' x ~requestRevision self = msg_send ~self ~cmd:(selector "initWithTensorsDictionary:requestRevision:") ~typ:(id @-> ullong @-> returning (id)) x requestRevision
