...
let applicableTrackerAndOptions x ~forRevision ~loadedInSession ~error self = msg_send ~self ~cmd:(selector "applicableTrackerAndOptions:forRevision:loadedInSession:error:") ~typ:(ptr (id) @-> ullong @-> id @-> ptr (id) @-> returning (id)) x forRevision loadedInSession error
