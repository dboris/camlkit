...
let _class_ = get_class "VNClustererReadWriteContext"
...
let initWithType x ~cachePath ~state ~threshold ~requestRevision ~error self = msg_send ~self ~cmd:(selector "initWithType:cachePath:state:threshold:requestRevision:error:") ~typ:(id @-> id @-> id @-> float @-> ullong @-> ptr (id) @-> returning (id)) x cachePath state threshold requestRevision error
...
let updateModelByAddingFaces2 x ~withGroupingIdentifiers ~andRemovingFaces ~canceller ~error self = msg_send ~self ~cmd:(selector "updateModelByAddingFaces:withGroupingIdentifiers:andRemovingFaces:canceller:error:") ~typ:(id @-> id @-> id @-> id @-> ptr (id) @-> returning (id)) x withGroupingIdentifiers andRemovingFaces canceller error
