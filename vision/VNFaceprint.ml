...
module Class = struct
  ...
  let emptyFaceprintDataForRevision x self = msg_send ~self ~cmd:(selector "emptyFaceprintDataForRevision:") ~typ:(ullong @-> returning (id)) x
...
let initWithRawData x ~confidence ~requestRevision self = msg_send ~self ~cmd:(selector "initWithRawData:confidence:requestRevision:") ~typ:(ptr (void) @-> float @-> ullong @-> returning (id)) x confidence requestRevision
