...
let initWithOriginatingRequestSpecifier x ~faceBoundingBox ~pointCount self = msg_send ~self ~cmd:(selector "initWithOriginatingRequestSpecifier:faceBoundingBox:pointCount:") ~typ:(id @-> CGRect.t @-> ullong @-> returning (id)) x faceBoundingBox pointCount
