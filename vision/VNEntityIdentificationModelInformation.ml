...
let initWithVersion x ~algorithm ~lastModificationDate ~readOnly self = msg_send ~self ~cmd:(selector "initWithVersion:algorithm:lastModificationDate:readOnly:") ~typ:(ullong @-> id @-> id @-> bool @-> returning (id)) x algorithm lastModificationDate readOnly
