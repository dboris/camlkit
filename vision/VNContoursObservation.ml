...
let childContourIndicesAtIndex x self = msg_send ~self ~cmd:(selector "childContourIndicesAtIndex:") ~typ:(llong @-> returning (ptr (void))) x
let contourAtIndex x ~error self = msg_send ~self ~cmd:(selector "contourAtIndex:error:") ~typ:(llong @-> ptr (id) @-> returning (id)) x error
