...
let initWithRequestRevision x ~boundingBox ~confidence ~labels self = msg_send ~self ~cmd:(selector "initWithRequestRevision:boundingBox:confidence:labels:") ~typ:(ullong @-> CGRect.t @-> float @-> id @-> returning (id)) x boundingBox confidence labels
