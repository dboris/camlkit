...
let _class_ = get_class "VNRecognizeAnimalsRequest"
...
module Class = struct
  ...
  let knownAnimalIdentifiersForRevision x ~error self = msg_send ~self ~cmd:(selector "knownAnimalIdentifiersForRevision:error:") ~typ:(ullong @-> ptr (id) @-> returning (id)) x error
