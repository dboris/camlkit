...
let initWithFaceIDModel x ~faceprintRequestRevision ~personUniqueIdentifierToSerialNumberMapping self = msg_send ~self ~cmd:(selector "initWithFaceIDModel:faceprintRequestRevision:personUniqueIdentifierToSerialNumberMapping:") ~typ:(ptr void @-> ullong @-> id @-> returning (id)) x (ULLong.of_int faceprintRequestRevision) personUniqueIdentifierToSerialNumberMapping
