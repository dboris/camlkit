...
let initWithVersion x ~lastModificationDate ~algorithm ~readOnly self = msg_send ~self ~cmd:(selector "initWithVersion:lastModificationDate:algorithm:readOnly:") ~typ:(ullong @-> id @-> id @-> bool @-> returning (id)) (ULLong.of_int x) lastModificationDate algorithm readOnly
