...
(** Apple docs: {{:https://developer.apple.com/documentation/photos/phimportrecord?language=objc}PHImportRecord} *)

let self = get_class "PHImportRecord"
...
let importType self = msg_send ~self ~cmd:(selector "importType") ~typ:(returning uchar)
...
let setImportType x self = msg_send ~self ~cmd:(selector "setImportType:") ~typ:(uchar @-> returning void) x
