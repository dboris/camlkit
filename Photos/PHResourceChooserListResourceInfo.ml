...
(** Apple docs: {{:https://developer.apple.com/documentation/photos/phresourcechooserlistresourceinfo?language=objc}PHResourceChooserListResourceInfo} *)

let self = get_class "PHResourceChooserListResourceInfo"

let approximateSize self = msg_send ~self ~cmd:(selector "approximateSize") ~typ:(returning CGSize.t)
