...
(** Apple docs: {{:https://developer.apple.com/documentation/photos/phimportdevicesource?language=objc}PHImportDeviceSource} *)

let self = get_class "PHImportDeviceSource"
...
let cameraDevice4 x ~setAccessState self = msg_send ~self ~cmd:(selector "cameraDevice:setAccessState:") ~typ:(id @-> uchar @-> returning void) x setAccessState
...
let cameraDevice6 x ~didReceiveThumbnail ~forItem ~error self = msg_send ~self ~cmd:(selector "cameraDevice:didReceiveThumbnail:forItem:error:") ~typ:(id @-> (ptr CGImage.t) @-> id @-> id @-> returning void) x didReceiveThumbnail forItem error
