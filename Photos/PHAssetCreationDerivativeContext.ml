...
(** Apple docs: {{:https://developer.apple.com/documentation/photos/phassetcreationderivativecontext?language=objc}PHAssetCreationDerivativeContext} *)

let self = get_class "PHAssetCreationDerivativeContext"
...
let assetOrientedImageSize self = msg_send ~self ~cmd:(selector "assetOrientedImageSize") ~typ:(returning CGSize.t)
...
let embeddedThumbnailSize self = msg_send ~self ~cmd:(selector "embeddedThumbnailSize") ~typ:(returning CGSize.t)
