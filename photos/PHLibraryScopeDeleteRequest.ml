...
let _class_ = get_class "PHLibraryScopeDeleteRequest"
...
let photosctlExpungeOverride self = msg_send ~self ~cmd:(selector "photosctlExpungeOverride") ~typ:(returning (bool))
let setOperation x self = msg_send ~self ~cmd:(selector "setOperation:") ~typ:(llong @-> returning (void)) (LLong.of_int x)
let setPhotosctlExpungeOverride x self = msg_send ~self ~cmd:(selector "setPhotosctlExpungeOverride:") ~typ:(bool @-> returning (void)) x
