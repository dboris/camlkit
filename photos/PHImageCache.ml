...
let _class_ = get_class "PHImageCache"
...
let populateEntryWithImage x ~requestID ~forKey ~additionalInfo self = msg_send ~self ~cmd:(selector "populateEntryWithImage:requestID:forKey:additionalInfo:") ~typ:(id @-> int @-> id @-> id @-> returning (bool)) x requestID forKey additionalInfo
