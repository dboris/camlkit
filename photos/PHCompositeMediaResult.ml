...
let _class_ = get_class "PHCompositeMediaResult"

module C = struct
  let resultWithRequestID x ~error self = msg_send ~self ~cmd:(selector "resultWithRequestID:error:") ~typ:(int @-> id @-> returning (id)) x error
end
