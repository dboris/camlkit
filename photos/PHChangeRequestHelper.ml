...
let _class_ = get_class "PHChangeRequestHelper"
...
let clientLinkedAgainst self = msg_send ~self ~cmd:(selector "clientLinkedAgainst") ~typ:(returning (id))
...
let setClientLinkedAgainst x self = msg_send ~self ~cmd:(selector "setClientLinkedAgainst:") ~typ:(id @-> returning (void)) x
