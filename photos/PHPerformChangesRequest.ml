...
let _class_ = get_class "PHPerformChangesRequest"
...
let didSendChangesToServiceWithResult x self = msg_send ~self ~cmd:(selector "didSendChangesToServiceWithResult:") ~typ:(bool @-> returning (void)) x
...
let executeWithLibraryServicesManager x ~libraryName ~executionContext ~reply self = msg_send ~self ~cmd:(selector "executeWithLibraryServicesManager:libraryName:executionContext:reply:") ~typ:(id @-> string @-> id @-> ptr void @-> returning (void)) x libraryName executionContext reply
...
let libraryRole self = msg_send ~self ~cmd:(selector "libraryRole") ~typ:(returning (ullong))
