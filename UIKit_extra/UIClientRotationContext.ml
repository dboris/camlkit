...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiclientrotationcontext?language=objc}UIClientRotationContext} *)

let self = get_class "UIClientRotationContext"
...
let initWithClient x ~toOrientation ~duration ~andWindow self = msg_send ~self ~cmd:(selector "initWithClient:toOrientation:duration:andWindow:") ~typ:(id @-> llong @-> double @-> id @-> returning id) x (LLong.of_int toOrientation) duration andWindow
...
let slideHeaderViewAndFooterViewOffScreen x ~forInterfaceOrientation self = msg_send ~self ~cmd:(selector "slideHeaderViewAndFooterViewOffScreen:forInterfaceOrientation:") ~typ:(bool @-> llong @-> returning void) x (LLong.of_int forInterfaceOrientation)
