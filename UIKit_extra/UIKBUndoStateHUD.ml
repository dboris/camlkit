...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikbundostatehud?language=objc}UIKBUndoStateHUD} *)

let self = get_class "UIKBUndoStateHUD"
...
let setControlType x self = msg_send ~self ~cmd:(selector "setControlType:") ~typ:(llong @-> returning void) (LLong.of_int x)
...
let updateUndoStateHUDControlType x ~available self = msg_send ~self ~cmd:(selector "updateUndoStateHUDControlType:available:") ~typ:(llong @-> bool @-> returning void) (LLong.of_int x) available