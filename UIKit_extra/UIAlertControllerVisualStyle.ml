...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uialertcontrollervisualstyle?language=objc}UIAlertControllerVisualStyle} *)

let self = get_class "UIAlertControllerVisualStyle"

let actionImageMarginForAction self = msg_send_stret ~self ~cmd:(selector "actionImageMarginForAction") ~typ:(returning UIEdgeInsets.t) ~return_type:UIEdgeInsets.t
