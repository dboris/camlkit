...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikeyboardautocorrectioncontroller?language=objc}UIKeyboardAutocorrectionController} *)

let self = get_class "UIKeyboardAutocorrectionController"
...
let setMaximumNumberOfCandidates x self = msg_send ~self ~cmd:(selector "setMaximumNumberOfCandidates:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
