...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uipickercolumnview?language=objc}UIPickerColumnView} *)
...
let initWithFrame x ~tableFrame ~middleBarHeight ~rowHeight ~pickerView ~transform self = msg_send ~self ~cmd:(selector "initWithFrame:tableFrame:middleBarHeight:rowHeight:pickerView:transform:") ~typ:(CGRect.t @-> CGRect.t @-> double @-> double @-> id @-> CATransform3D.t @-> returning id) x tableFrame middleBarHeight rowHeight pickerView transform
...
let setPerspectiveTransform x self = msg_send ~self ~cmd:(selector "setPerspectiveTransform:") ~typ:(CATransform3D.t @-> returning void) x
