...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiinterfaceactionconcretevisualstyle?language=objc}UIInterfaceActionConcreteVisualStyle} *)

let self = get_class "UIInterfaceActionConcreteVisualStyle"
...
let actionSequenceEdgeInsets self = msg_send_stret ~self ~cmd:(selector "actionSequenceEdgeInsets") ~typ:(returning UIEdgeInsets.t) ~return_type:UIEdgeInsets.t
...
let maximumActionGroupContentSize self = msg_send_stret ~self ~cmd:(selector "maximumActionGroupContentSize") ~typ:(returning CGSize.t) ~return_type:CGSize.t
