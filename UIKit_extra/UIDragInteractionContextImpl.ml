...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uidraginteractioncontextimpl?language=objc}UIDragInteractionContextImpl} *)

let self = get_class "UIDragInteractionContextImpl"
...
let initialLocation self = msg_send ~self ~cmd:(selector "initialLocation") ~typ:(returning CGPoint.t)
