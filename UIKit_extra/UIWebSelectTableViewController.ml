...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiwebselecttableviewcontroller?language=objc}UIWebSelectTableViewController} *)

let self = get_class "UIWebSelectTableViewController"
...
let initWithDOMHTMLSelectNode x ~cachedItems ~singleSelectionIndex ~multipleSelection self = msg_send ~self ~cmd:(selector "initWithDOMHTMLSelectNode:cachedItems:singleSelectionIndex:multipleSelection:") ~typ:(id @-> id @-> ullong @-> bool @-> returning id) x cachedItems (ULLong.of_int singleSelectionIndex) multipleSelection
