...
let collectionView2 x ~layout ~insetForSectionAtIndex self = msg_send_stret ~self ~cmd:(selector "collectionView:layout:insetForSectionAtIndex:") ~typ:(id @-> id @-> llong @-> returning UIEdgeInsets.t) ~return_type:UIEdgeInsets.t x layout (LLong.of_int insetForSectionAtIndex)
