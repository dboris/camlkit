...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikbsplitrow?language=objc}UIKBSplitRow} *)

let self = get_class "UIKBSplitRow"
...
let rowIndex self = msg_send ~self ~cmd:(selector "rowIndex") ~typ:(returning uchar)
...
let setRowIndex x self = msg_send ~self ~cmd:(selector "setRowIndex:") ~typ:(uchar @-> returning void) x
