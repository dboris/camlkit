...
let _class_ = get_class "UIDebuggingZoomViewController"
...
let newCaptureSnapshotAtRect x ~window self = msg_send ~self ~cmd:(selector "newCaptureSnapshotAtRect:window:") ~typ:(CGRect.t @-> id @-> returning (id)) x window
...
let setCurrentDirection x self = msg_send ~self ~cmd:(selector "setCurrentDirection:") ~typ:(ullong @-> returning (void)) x
