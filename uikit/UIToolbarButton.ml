...
let initWithImage x ~pressedImage ~label ~labelHeight ~withBarStyle ~withStyle ~withInsets ~possibleTitles ~possibleSystemItems ~withToolbarTintColor ~bezel ~imageInsets ~glowInsets ~landscape self = msg_send ~self ~cmd:(selector "initWithImage:pressedImage:label:labelHeight:withBarStyle:withStyle:withInsets:possibleTitles:possibleSystemItems:withToolbarTintColor:bezel:imageInsets:glowInsets:landscape:") ~typ:(id @-> id @-> id @-> double @-> llong @-> llong @-> ptr void @-> id @-> id @-> id @-> bool @-> ptr void @-> ptr void @-> bool @-> returning (id)) x pressedImage label labelHeight (LLong.of_int withBarStyle) (LLong.of_int withStyle) withInsets possibleTitles possibleSystemItems withToolbarTintColor bezel imageInsets glowInsets landscape
