...
let _class_ = get_class "UIViewControllerBuiltinTransitionViewAnimator"
...
let transitionView1 x ~beginOriginForToView ~forTransition ~defaultOrigin self = msg_send_stret ~self ~cmd:(selector "transitionView:beginOriginForToView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning (CGPoint.t)) ~return_type:CGPoint.t x beginOriginForToView forTransition defaultOrigin
let transitionView2 x ~endOriginForFromView ~forTransition ~defaultOrigin self = msg_send_stret ~self ~cmd:(selector "transitionView:endOriginForFromView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning (CGPoint.t)) ~return_type:CGPoint.t x endOriginForFromView forTransition defaultOrigin
let transitionView3 x ~endOriginForToView ~forTransition ~defaultOrigin self = msg_send_stret ~self ~cmd:(selector "transitionView:endOriginForToView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning (CGPoint.t)) ~return_type:CGPoint.t x endOriginForToView forTransition defaultOrigin
