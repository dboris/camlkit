...
let pushDirection self = msg_send ~self ~cmd:(selector "pushDirection") ~typ:(returning CGVector.t)
...
let targetOffsetFromCenterForItem x self = msg_send ~self ~cmd:(selector "targetOffsetFromCenterForItem:") ~typ:(id @-> returning UIOffset.t) x
