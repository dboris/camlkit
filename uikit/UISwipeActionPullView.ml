...
let _class_ = get_class "UISwipeActionPullView"
...
let initWithFrame x ~cellEdge ~style self = msg_send ~self ~cmd:(selector "initWithFrame:cellEdge:style:") ~typ:(CGRect.t @-> ullong @-> ullong @-> returning (id)) x cellEdge style
...
let setState x self = msg_send ~self ~cmd:(selector "setState:") ~typ:(ullong @-> returning (void)) x
