...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikeyboardkeyswipetransition?language=objc}UIKeyboardKeySwipeTransition} *)

let self = get_class "UIKeyboardKeySwipeTransition"
...
let keyRectForFrame x ~normalizedSubRect self = msg_send ~self ~cmd:(selector "keyRectForFrame:normalizedSubRect:") ~typ:(CGRect.t @-> CGRect.t @-> returning CGRect.t) x normalizedSubRect
