...
let _class_ = get_class "UIKeyboardPathEffectView"

let accessibilityValueChanged x self = msg_send ~self ~cmd:(selector "accessibilityValueChanged:") ~typ:(id @-> returning (void)) x
let addPoint x ~force ~timestamp self = msg_send ~self ~cmd:(selector "addPoint:force:timestamp:") ~typ:(CGPoint.t @-> double @-> double @-> returning (void)) x force timestamp
