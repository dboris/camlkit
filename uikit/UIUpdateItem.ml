...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiupdateitem?language=objc}UIUpdateItem} *)

let self = get_class "UIUpdateItem"
...
let initWithAction x ~forIndexPath ~animation self = msg_send ~self ~cmd:(selector "initWithAction:forIndexPath:animation:") ~typ:(int @-> id @-> llong @-> returning id) x forIndexPath (LLong.of_int animation)
...
let setAnimation x self = msg_send ~self ~cmd:(selector "setAnimation:") ~typ:(llong @-> returning void) (LLong.of_int x)
