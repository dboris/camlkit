...
let _class_ = get_class "UIScenePresentationBinder"
...
let initWithIdentifier x ~priority ~rootView ~appearanceStyle self = msg_send ~self ~cmd:(selector "initWithIdentifier:priority:rootView:appearanceStyle:") ~typ:(id @-> llong @-> id @-> ullong @-> returning (id)) x priority rootView appearanceStyle
