...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikeyboardsplittransitionview?language=objc}UIKeyboardSplitTransitionView} *)

let self = get_class "UIKeyboardSplitTransitionView"
...
let keyImageWithToken x self = msg_send ~self ~cmd:(selector "keyImageWithToken:") ~typ:(id @-> returning (ptr CGImage.t)) x
...
let rectEnclosingKeyplane x self = msg_send ~self ~cmd:(selector "rectEnclosingKeyplane:") ~typ:(id @-> returning CGRect.t) x
...
let sizeForShadowLayer x self = msg_send ~self ~cmd:(selector "sizeForShadowLayer:") ~typ:(int @-> returning CGSize.t) x
