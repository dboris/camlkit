...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uipressinfo?language=objc}UIPressInfo} *)

let self = get_class "UIPressInfo"
...
let setGameControllerComponent x self = msg_send ~self ~cmd:(selector "setGameControllerComponent:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
...
let setSource x self = msg_send ~self ~cmd:(selector "setSource:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
