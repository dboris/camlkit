...
let _class_ = get_class "UIDebuggingSpecViewController"
...
let collectionView3 x ~numberOfItemsInSection self = msg_send ~self ~cmd:(selector "collectionView:numberOfItemsInSection:") ~typ:(id @-> llong @-> returning (llong)) x numberOfItemsInSection
