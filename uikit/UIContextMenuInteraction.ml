...
let _class_ = get_class "UIContextMenuInteraction"
...
let stashedLocation self = msg_send ~self ~cmd:(selector "stashedLocation") ~typ:(returning (CGPoint.t))
let targetedPreviewForAnimator x ~dismissingWithStyle ~clientReturnedPreview self = msg_send ~self ~cmd:(selector "targetedPreviewForAnimator:dismissingWithStyle:clientReturnedPreview:") ~typ:(id @-> ullong @-> ptr (bool) @-> returning (id)) x dismissingWithStyle clientReturnedPreview
