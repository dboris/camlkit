...
let addOptionWithTitle x ~image ~order ~handler self = msg_send ~self ~cmd:(selector "addOptionWithTitle:image:order:handler:") ~typ:(id @-> id @-> ullong @-> ptr void @-> returning (void)) x image order handler
