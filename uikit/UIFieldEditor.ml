...
let self = get_class "UIFieldEditor"
...
let setPadding x self = msg_send ~self ~cmd:(selector "setPadding:") ~typ:(UIEdgeInsets.t @-> returning void) x
...
let textContainerOriginForTextAlignment x self = msg_send ~self ~cmd:(selector "textContainerOriginForTextAlignment:") ~typ:(llong @-> returning CGPoint.t) (LLong.of_int x)
