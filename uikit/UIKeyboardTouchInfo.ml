...
let _class_ = get_class "UIKeyboardTouchInfo"
...
let initialDragPoint self = msg_send ~self ~cmd:(selector "initialDragPoint") ~typ:(returning (CGPoint.t))
...
let setContinuousPathState x self = msg_send ~self ~cmd:(selector "setContinuousPathState:") ~typ:(llong @-> returning (void)) x
...
let setProcessedTouchCount x self = msg_send ~self ~cmd:(selector "setProcessedTouchCount:") ~typ:(ullong @-> returning (void)) x
