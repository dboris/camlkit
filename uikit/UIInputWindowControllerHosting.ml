...
let disableViewSizingConstraints x ~forNewView self = msg_send ~self ~cmd:(selector "disableViewSizingConstraints:forNewView:") ~typ:(ullong @-> id @-> returning (void)) x forNewView
...
let host x ~isForPurpose self = msg_send ~self ~cmd:(selector "host:isForPurpose:") ~typ:(id @-> ullong @-> returning (bool)) x isForPurpose
...
let itemForPurpose x self = msg_send ~self ~cmd:(selector "itemForPurpose:") ~typ:(ullong @-> returning (id)) x
