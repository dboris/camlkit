...
let _class_ = get_class "UIStatusBarForegroundView"
...
let frameForAllItemsInRegion x self = msg_send_stret ~self ~cmd:(selector "frameForAllItemsInRegion:") ~typ:(int @-> returning (CGRect.t)) ~return_type:CGRect.t x
let frameForItemOfType x self = msg_send_stret ~self ~cmd:(selector "frameForItemOfType:") ~typ:(int @-> returning (CGRect.t)) ~return_type:CGRect.t x
...
let setIdiom x self = msg_send ~self ~cmd:(selector "setIdiom:") ~typ:(llong @-> returning (void)) x
