...
let _class_ = get_class "UIRevealInteractionHighlighter"

let completeHighlightingItem x self = msg_send ~self ~cmd:(selector "completeHighlightingItem:") ~typ:(id @-> returning (void)) x
let drawHighlightContentForItem x ~context self = msg_send ~self ~cmd:(selector "drawHighlightContentForItem:context:") ~typ:(id @-> id @-> returning (void)) x context
