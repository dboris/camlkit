...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiinterfaceactiongroupview?language=objc}UIInterfaceActionGroupView} *)

let self = get_class "UIInterfaceActionGroupView"
...
let insertArrangedHeaderView x ~atIndex ~scrollable self = msg_send ~self ~cmd:(selector "insertArrangedHeaderView:atIndex:scrollable:") ~typ:(id @-> ullong @-> bool @-> returning void) x (ULLong.of_int atIndex) scrollable
...
let setActionLayoutAxis x self = msg_send ~self ~cmd:(selector "setActionLayoutAxis:") ~typ:(llong @-> returning void) (LLong.of_int x)
...
let setPresentationStyle x self = msg_send ~self ~cmd:(selector "setPresentationStyle:") ~typ:(llong @-> returning void) (LLong.of_int x)
