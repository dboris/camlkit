...
let self = get_class "UIWindowController"
...
let transitionView2 x ~beginOriginForToView ~forTransition ~defaultOrigin self = msg_send ~self ~cmd:(selector "transitionView:beginOriginForToView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning CGPoint.t) x beginOriginForToView forTransition defaultOrigin
let transitionView3 x ~endOriginForFromView ~forTransition ~defaultOrigin self = msg_send ~self ~cmd:(selector "transitionView:endOriginForFromView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning CGPoint.t) x endOriginForFromView forTransition defaultOrigin
let transitionView4 x ~endOriginForToView ~forTransition ~defaultOrigin self = msg_send ~self ~cmd:(selector "transitionView:endOriginForToView:forTransition:defaultOrigin:") ~typ:(id @-> id @-> int @-> CGPoint.t @-> returning CGPoint.t) x endOriginForToView forTransition defaultOrigin
