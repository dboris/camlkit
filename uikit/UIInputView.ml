...
let initWithFrame' x ~inputViewStyle self = msg_send ~self ~cmd:(selector "initWithFrame:inputViewStyle:") ~typ:(CGRect.t @-> llong @-> returning (id)) x inputViewStyle
...
let setInputViewStyle x self = msg_send ~self ~cmd:(selector "setInputViewStyle:") ~typ:(llong @-> returning (void)) x
