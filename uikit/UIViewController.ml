...
let additionalSafeAreaInsets self = msg_send ~self ~cmd:(selector "additionalSafeAreaInsets") ~typ:(returning UIEdgeInsets.t)
...
let systemMinimumLayoutMargins self = msg_send ~self ~cmd:(selector "systemMinimumLayoutMargins") ~typ:(returning NSDirectionalEdgeInsets.t)
...
let tranformForScreenOriginRotation x self = msg_send ~self ~cmd:(selector "tranformForScreenOriginRotation:") ~typ:(float @-> returning CGAffineTransform.t) x
