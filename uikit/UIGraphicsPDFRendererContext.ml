...
let _class_ = get_class "UIGraphicsPDFRendererContext"
...
let pageBounds self = msg_send ~self ~cmd:(selector "pageBounds") ~typ:(returning (CGRect.t))
let pdfContextBounds self = msg_send ~self ~cmd:(selector "pdfContextBounds") ~typ:(returning (CGRect.t))
