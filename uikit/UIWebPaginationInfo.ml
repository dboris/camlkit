...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uiwebpaginationinfo?language=objc}UIWebPaginationInfo} *)

let self = get_class "UIWebPaginationInfo"
...
let layoutSize self = msg_send ~self ~cmd:(selector "layoutSize") ~typ:(returning CGSize.t)
...
let sizeForPageAtIndex x self = msg_send ~self ~cmd:(selector "sizeForPageAtIndex:") ~typ:(llong @-> returning CGSize.t) (LLong.of_int x)
