...
let self = get_class "UINavigationItem"
...
let largeTitleInsets self = msg_send ~self ~cmd:(selector "largeTitleInsets") ~typ:(returning NSDirectionalEdgeInsets.t)
...
let setLargeTitleInsets x self = msg_send ~self ~cmd:(selector "setLargeTitleInsets:") ~typ:(NSDirectionalEdgeInsets.t @-> returning void) x
