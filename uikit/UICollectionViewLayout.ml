...
let self = get_class "UICollectionViewLayout"
...
let convertRect x ~fromLayout self = msg_send ~self ~cmd:(selector "convertRect:fromLayout:") ~typ:(CGRect.t @-> id @-> returning CGRect.t) x fromLayout
let convertRect' x ~toLayout self = msg_send ~self ~cmd:(selector "convertRect:toLayout:") ~typ:(CGRect.t @-> id @-> returning CGRect.t) x toLayout
