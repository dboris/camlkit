...
let _class_ = get_class "UIDragItem"
...
let setPreferredPreviewType x self = msg_send ~self ~cmd:(selector "setPreferredPreviewType:") ~typ:(ullong @-> returning (void)) x
...
let setUpdatedPreviewType x self = msg_send ~self ~cmd:(selector "setUpdatedPreviewType:") ~typ:(ullong @-> returning (void)) x
