...
module Class = struct
  ...
  let sliceSetIDForKeyplaneName x ~type_ ~orientation self = msg_send ~self ~cmd:(selector "sliceSetIDForKeyplaneName:type:orientation:") ~typ:(id @-> llong @-> llong @-> returning (id)) x type_ orientation
end
