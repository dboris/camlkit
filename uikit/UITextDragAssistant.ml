...
let _class_ = get_class "UITextDragAssistant"
...
let dragInteraction9 x ~session ~didEndWithOperation self = msg_send ~self ~cmd:(selector "dragInteraction:session:didEndWithOperation:") ~typ:(id @-> id @-> ullong @-> returning (void)) x session didEndWithOperation
let dragInteraction10 x ~session ~willEndWithOperation self = msg_send ~self ~cmd:(selector "dragInteraction:session:willEndWithOperation:") ~typ:(id @-> id @-> ullong @-> returning (void)) x session willEndWithOperation
