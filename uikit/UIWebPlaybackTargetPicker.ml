...
let showAirPlayPickerIPad x ~fromRect self = msg_send ~self ~cmd:(selector "showAirPlayPickerIPad:fromRect:") ~typ:(llong @-> CGRect.t @-> returning (void)) x fromRect
let showAirPlayPickerIPhone x self = msg_send ~self ~cmd:(selector "showAirPlayPickerIPhone:") ~typ:(llong @-> returning (void)) x