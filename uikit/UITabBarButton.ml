...
let initWithImage x ~selectedImage ~label ~withInsets ~tabBar self = msg_send ~self ~cmd:(selector "initWithImage:selectedImage:label:withInsets:tabBar:") ~typ:(id @-> id @-> id @-> ptr void @-> id @-> returning (id)) x selectedImage label withInsets tabBar
