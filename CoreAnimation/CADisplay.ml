...
let overscanAmounts self = msg_send ~self ~cmd:(selector "overscanAmounts") ~typ:(returning CGSize.t)
...
let safeBounds self = msg_send ~self ~cmd:(selector "safeBounds") ~typ:(returning CGRect.t)
