...
(** Apple docs: {{:https://developer.apple.com/documentation/foundation/nsfileproviderproxy?language=objc}NSFileProviderProxy} *)

let self = get_class "NSFileProviderProxy"
...
let provideItemAtURL x ~withOptions ~forAccessClaim ~completionHandler self = msg_send ~self ~cmd:(selector "provideItemAtURL:withOptions:forAccessClaim:completionHandler:") ~typ:(id @-> ullong @-> id @-> (ptr void) @-> returning (ptr void)) x (ULLong.of_int withOptions) forAccessClaim completionHandler
let provideItemForKernelRequestWithInfo x ~atURL ~forProcess ~withOptions ~completionHandler self = msg_send ~self ~cmd:(selector "provideItemForKernelRequestWithInfo:atURL:forProcess:withOptions:completionHandler:") ~typ:(id @-> id @-> int @-> ullong @-> (ptr void) @-> returning (ptr void)) x atURL forProcess (ULLong.of_int withOptions) completionHandler
