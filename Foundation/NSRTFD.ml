...
let insertItem x ~path ~dirInfo ~zone ~plist self = msg_send ~self ~cmd:(selector "insertItem:path:dirInfo:zone:plist:") ~typ:(id @-> id @-> id @-> (ptr void) @-> id @-> returning ullong) x path dirInfo zone plist
