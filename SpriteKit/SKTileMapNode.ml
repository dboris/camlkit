...
let centerOfTileAtColumn x ~row self = msg_send_stret ~self ~cmd:(selector "centerOfTileAtColumn:row:") ~typ:(ullong @-> ullong @-> returning CGPoint.t) ~return_type:CGPoint.t (ULLong.of_int x) (ULLong.of_int row)
...
let mapSize self = msg_send_stret ~self ~cmd:(selector "mapSize") ~typ:(returning CGSize.t) ~return_type:CGSize.t
