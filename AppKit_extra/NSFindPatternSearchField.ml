...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsfindpatternsearchfield?language=objc}NSFindPatternSearchField} *)

let self = get_class "NSFindPatternSearchField"
...
let textView2 x ~clickedOnCell ~inRect ~atIndex self = msg_send ~self ~cmd:(selector "textView:clickedOnCell:inRect:atIndex:") ~typ:(id @-> id @-> CGRect.t @-> ullong @-> returning void) x clickedOnCell inRect (ULLong.of_int atIndex)
let textView3 x ~doubleClickedOnCell ~inRect ~atIndex self = msg_send ~self ~cmd:(selector "textView:doubleClickedOnCell:inRect:atIndex:") ~typ:(id @-> id @-> CGRect.t @-> ullong @-> returning void) x doubleClickedOnCell inRect (ULLong.of_int atIndex)