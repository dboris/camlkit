...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nscgswindowbuffer?language=objc}NSCGSWindowBuffer} *)

let self = get_class "NSCGSWindowBuffer"
...
let initWithBytes x ~width ~height ~bytesPerRow ~bitsPerPixel ~bitsPerComponent ~bitmapInfo ~colorSpace ~scale self = msg_send ~self ~cmd:(selector "initWithBytes:width:height:bytesPerRow:bitsPerPixel:bitsPerComponent:bitmapInfo:colorSpace:scale:") ~typ:((ptr void) @-> ullong @-> ullong @-> llong @-> ullong @-> ullong @-> uint @-> (ptr CGColorSpace.t) @-> double @-> returning id) x (ULLong.of_int width) (ULLong.of_int height) (LLong.of_int bytesPerRow) (ULLong.of_int bitsPerPixel) (ULLong.of_int bitsPerComponent) bitmapInfo colorSpace scale
