...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsstackviewcontainer?language=objc}NSStackViewContainer} *)

let self = get_class "NSStackViewContainer"
...
let insertView x ~atIndex self = msg_send ~self ~cmd:(selector "insertView:atIndex:") ~typ:(id @-> ullong @-> returning void) x (ULLong.of_int atIndex)
