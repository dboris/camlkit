...
let self = get_class "NSScrollerImp"
...
let expandedRectForPart x self = msg_send ~self ~cmd:(selector "expandedRectForPart:") ~typ:(ullong @-> returning CGRect.t) (ULLong.of_int x)
...
let rectForPart x self = msg_send ~self ~cmd:(selector "rectForPart:") ~typ:(ullong @-> returning CGRect.t) (ULLong.of_int x)
