...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsmenuwindowmanagerwindow?language=objc}NSMenuWindowManagerWindow} *)

let self = get_class "NSMenuWindowManagerWindow"
...
let flippedContainerViewFrame self = msg_send_stret ~self ~cmd:(selector "flippedContainerViewFrame") ~typ:(returning CGRect.t) ~return_type:CGRect.t
...
let setWindowRef x self = msg_send ~self ~cmd:(selector "setWindowRef:") ~typ:((ptr void) @-> returning void) x
