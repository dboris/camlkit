...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nstouchbarcolorlistpickerpressandholdpopup?language=objc}NSTouchBarColorListPickerPressAndHoldPopUp} *)

let self = get_class "NSTouchBarColorListPickerPressAndHoldPopUp"
...
let scrubber' x ~viewForItemAtIndex self = msg_send ~self ~cmd:(selector "scrubber:viewForItemAtIndex:") ~typ:(id @-> llong @-> returning id) x (LLong.of_int viewForItemAtIndex)
...
let setNumberOfDarkerColors x self = msg_send ~self ~cmd:(selector "setNumberOfDarkerColors:") ~typ:(llong @-> returning void) (LLong.of_int x)
let setNumberOfLighterColors x self = msg_send ~self ~cmd:(selector "setNumberOfLighterColors:") ~typ:(llong @-> returning void) (LLong.of_int x)
