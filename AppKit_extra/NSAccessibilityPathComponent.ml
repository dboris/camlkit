...
let self = get_class "NSAccessibilityPathComponent"
...
let initWithPathComponentCell x ~index ~parent self = msg_send ~self ~cmd:(selector "initWithPathComponentCell:index:parent:") ~typ:(id @-> llong @-> id @-> returning id) x (LLong.of_int index) parent
