...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nstoolbaritemviewer?language=objc}NSToolbarItemViewer} *)

let self = get_class "NSToolbarItemViewer"
...
let itemViewRect self = msg_send_stret ~self ~cmd:(selector "itemViewRect") ~typ:(returning CGRect.t) ~return_type:CGRect.t
...
let minViewerSize self = msg_send_stret ~self ~cmd:(selector "minViewerSize") ~typ:(returning CGSize.t) ~return_type:CGSize.t
