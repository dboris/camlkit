...
let self = get_class "NSCarbonWindow"
...
let carbonPlatformWindowBounds self = msg_send_stret ~self ~cmd:(selector "carbonPlatformWindowBounds") ~typ:(returning CGRect.t) ~return_type:CGRect.t
...
let sendCarbonUpdateHICommandStatusEvent x ~withMenuRef ~andMenuItemIndex self = msg_send ~self ~cmd:(selector "sendCarbonUpdateHICommandStatusEvent:withMenuRef:andMenuItemIndex:") ~typ:(uint @-> (ptr void) @-> ushort @-> returning uchar) x withMenuRef andMenuItemIndex
