...
let self = get_class "NSPersistentUIRecord"
...
let copyData self = msg_send ~self ~cmd:(selector "copyData") ~typ:(returning (ptr CFData.t))
...
let setEncryptionKey x self = msg_send ~self ~cmd:(selector "setEncryptionKey:") ~typ:((ptr uchar) @-> returning void) x
