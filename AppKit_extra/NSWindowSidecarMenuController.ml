...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nswindowsidecarmenucontroller?language=objc}NSWindowSidecarMenuController} *)

let self = get_class "NSWindowSidecarMenuController"
...
let requestZoomToDisplayID x self = msg_send ~self ~cmd:(selector "requestZoomToDisplayID:") ~typ:(llong @-> returning void) (LLong.of_int x)
...
let zoomToScreenWithDisplayID x self = msg_send ~self ~cmd:(selector "zoomToScreenWithDisplayID:") ~typ:(llong @-> returning bool) (LLong.of_int x)