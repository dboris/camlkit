...
let sizeOfTitlebarButtons self = msg_send_stret ~self ~cmd:(selector "sizeOfTitlebarButtons") ~typ:(returning CGSize.t) ~return_type:CGSize.t
...
let titlebarRectIncludingToolbar self = msg_send_stret ~self ~cmd:(selector "titlebarRectIncludingToolbar") ~typ:(returning CGRect.t) ~return_type:CGRect.t
