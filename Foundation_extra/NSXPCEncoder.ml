...
(** Apple docs: {{:https://developer.apple.com/documentation/foundation/nsxpcencoder?language=objc}NSXPCEncoder} *)

let self = get_class "NSXPCEncoder"
...
let initWithStackSpace x ~size self = msg_send ~self ~cmd:(selector "initWithStackSpace:size:") ~typ:(string @-> ullong @-> returning id) x (ULLong.of_int size)
