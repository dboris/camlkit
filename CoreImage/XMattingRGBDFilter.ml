...
(** Apple docs: {{:https://developer.apple.com/documentation/coreimage/xmattingrgbdfilter?language=objc}XMattingRGBDFilter} *)

let self = get_class "XMattingRGBDFilter"
...
let encodeDisparityToAlphaOn x ~disparityTexture ~destinationTexture self = msg_send ~self ~cmd:(selector "encodeDisparityToAlphaOn:disparityTexture:destinationTexture:") ~typ:(id @-> id @-> id @-> returning int) x disparityTexture destinationTexture
let encodeNearestNeighborDownSamplingOn x ~inputTexture ~outputTexture self = msg_send ~self ~cmd:(selector "encodeNearestNeighborDownSamplingOn:inputTexture:outputTexture:") ~typ:(id @-> id @-> id @-> returning int) x inputTexture outputTexture
let encodePushOn x ~textureArray ~texture self = msg_send ~self ~cmd:(selector "encodePushOn:textureArray:texture:") ~typ:(id @-> id @-> id @-> returning void) x textureArray texture
