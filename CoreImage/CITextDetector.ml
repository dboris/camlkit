...
let ctmForImageWithBounds x ~orientation self = msg_send_stret ~self ~cmd:(selector "ctmForImageWithBounds:orientation:") ~typ:(CGRect.t @-> int @-> returning CGAffineTransform.t) ~return_type:CGAffineTransform.t x orientation
