...
(** Apple docs: {{:https://developer.apple.com/documentation/coreimage/ciportraitblurv2?language=objc}CIPortraitBlurV2} *)

let self = get_class "CIPortraitBlurV2"

let effectiveScale self = msg_send ~self ~cmd:(selector "effectiveScale") ~typ:(returning double)
...
let sensorSize self = msg_send ~self ~cmd:(selector "sensorSize") ~typ:(returning id)
