...
(** Apple docs: {{:https://developer.apple.com/documentation/coreimage/cibilateralsolvercpu?language=objc}CIBilateralSolverCPU} *)

let self = get_class "CIBilateralSolverCPU"

let dealloc self = msg_send ~self ~cmd:(selector "dealloc") ~typ:(returning void)
let doSolveWithBilateralGridhash x ~reference ~disparity ~confidence ~output ~lambda ~maxIterations self = msg_send ~self ~cmd:(selector "doSolveWithBilateralGridhash:reference:disparity:confidence:output:lambda:maxIterations:") ~typ:(id @-> (ptr IOSurface.t) @-> (ptr void) @-> (ptr void) @-> (ptr void) @-> float @-> int @-> returning int) x reference disparity confidence output lambda maxIterations
