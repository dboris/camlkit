...
(** Apple docs: {{:https://developer.apple.com/documentation/coreimage/ciperspectivecorrection?language=objc}CIPerspectiveCorrection} *)

let self = get_class "CIPerspectiveCorrection"
...
let inputCrop self = msg_send ~self ~cmd:(selector "inputCrop") ~typ:(returning id)
...
let setInputCrop x self = msg_send ~self ~cmd:(selector "setInputCrop:") ~typ:(id @-> returning void) x
