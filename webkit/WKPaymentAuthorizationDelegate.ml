...
let _class_ = get_class "WKPaymentAuthorizationDelegate"
...
let completePaymentSession x ~errors self = msg_send ~self ~cmd:(selector "completePaymentSession:errors:") ~typ:(llong @-> id @-> returning (void)) x errors
