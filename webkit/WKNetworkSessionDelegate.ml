...
let _URLSession9 x ~downloadTask ~didResumeAtOffset ~expectedTotalBytes self = msg_send ~self ~cmd:(selector "URLSession:downloadTask:didResumeAtOffset:expectedTotalBytes:") ~typ:(id @-> id @-> llong @-> llong @-> returning (void)) x downloadTask didResumeAtOffset expectedTotalBytes
...
let _URLSession11 x ~webSocketTask ~didCloseWithCode ~reason self = msg_send ~self ~cmd:(selector "URLSession:webSocketTask:didCloseWithCode:reason:") ~typ:(id @-> id @-> llong @-> id @-> returning (void)) x webSocketTask didCloseWithCode reason
...
let _URLSession13 x ~task ~didSendBodyData ~totalBytesSent ~totalBytesExpectedToSend self = msg_send ~self ~cmd:(selector "URLSession:task:didSendBodyData:totalBytesSent:totalBytesExpectedToSend:") ~typ:(id @-> id @-> llong @-> llong @-> llong @-> returning (void)) x task didSendBodyData totalBytesSent totalBytesExpectedToSend
