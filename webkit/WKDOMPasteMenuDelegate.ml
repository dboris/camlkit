...
let _class_ = get_class "WKDOMPasteMenuDelegate"
...
let initWithWebViewImpl x ~pasteAccessCategory self = msg_send ~self ~cmd:(selector "initWithWebViewImpl:pasteAccessCategory:") ~typ:(ptr (void) @-> bool @-> returning (id)) x pasteAccessCategory
