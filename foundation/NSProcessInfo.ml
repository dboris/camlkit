...
let _class_ = get_class "NSProcessInfo"
...
let isOperatingSystemAtLeastVersion x self = msg_send ~self ~cmd:(selector "isOperatingSystemAtLeastVersion:") ~typ:(ptr void @-> returning (bool)) x
...
let macCatalystVersionIsAtLeastVersion x self = msg_send ~self ~cmd:(selector "macCatalystVersionIsAtLeastVersion:") ~typ:(ptr void @-> returning (bool)) x
