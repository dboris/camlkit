...
let _class_ = get_class "NSItemProviderRepresentation"
...
let initWithType ~x ~preferredRepresentation ~loader self = msg_send ~self ~cmd:(selector "initWithType:preferredRepresentation:loader:") ~typ:(id @-> llong @-> ptr void @-> returning (id)) x preferredRepresentation loader
...
let setPreferredRepresentation ~x self = msg_send ~self ~cmd:(selector "setPreferredRepresentation:") ~typ:(llong @-> returning (void)) x
let setVisibility ~x self = msg_send ~self ~cmd:(selector "setVisibility:") ~typ:(llong @-> returning (void)) x
