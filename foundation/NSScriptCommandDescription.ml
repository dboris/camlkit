...
let createCommandInstanceWithZone ~x self = msg_send ~self ~cmd:(selector "createCommandInstanceWithZone:") ~typ:(id @-> returning (id)) x
...
let isOptionalArgumentWithName ~x self = msg_send ~self ~cmd:(selector "isOptionalArgumentWithName:") ~typ:(id @-> returning (char)) x
