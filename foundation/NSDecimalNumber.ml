...
let charValue  self = msg_send ~self ~cmd:(selector "charValue") ~typ:(returning (char))
...
let initWithMantissa ~x ~exponent ~isNegative self = msg_send ~self ~cmd:(selector "initWithMantissa:exponent:isNegative:") ~typ:(ullong @-> short @-> char @-> returning (id)) x exponent isNegative
