...
let setWantsWriteNotifications ~x self = msg_send ~self ~cmd:(selector "setWantsWriteNotifications:") ~typ:(char @-> returning (void)) x
let uniqueID  self = msg_send ~self ~cmd:(selector "uniqueID") ~typ:(returning (id)) 
let wantsWriteNotifications  self = msg_send ~self ~cmd:(selector "wantsWriteNotifications") ~typ:(returning (char)) 