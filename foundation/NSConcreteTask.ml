...
let launchWithDictionary ~x ~error self = msg_send ~self ~cmd:(selector "launchWithDictionary:error:") ~typ:(id @-> ptr (id) @-> returning (char)) x error
...
let setStartsNewProcessGroup ~x self = msg_send ~self ~cmd:(selector "setStartsNewProcessGroup:") ~typ:(char @-> returning (void)) x
