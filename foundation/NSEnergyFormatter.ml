...
let isForFoodEnergyUse  self = msg_send ~self ~cmd:(selector "isForFoodEnergyUse") ~typ:(returning (char)) 
...
let setForFoodEnergyUse ~x self = msg_send ~self ~cmd:(selector "setForFoodEnergyUse:") ~typ:(char @-> returning (void)) x
