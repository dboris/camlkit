...
let postNotificationName3 ~x ~object_ ~userInfo ~deliverImmediately self = msg_send ~self ~cmd:(selector "postNotificationName:object:userInfo:deliverImmediately:") ~typ:(id @-> id @-> id @-> char @-> returning (void)) x object_ userInfo deliverImmediately
let postNotificationName2 ~x ~object_ ~userInfo ~options self = msg_send ~self ~cmd:(selector "postNotificationName:object:userInfo:options:") ~typ:(id @-> id @-> id @-> ullong @-> returning (void)) x object_ userInfo options
...
let suspended  self = msg_send ~self ~cmd:(selector "suspended") ~typ:(returning (char))