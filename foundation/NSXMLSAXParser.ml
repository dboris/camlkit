...
let _class_ = get_class "NSXMLSAXParser"
...
let initWithData' x ~isSingleDTDNode ~options ~error self = msg_send ~self ~cmd:(selector "initWithData:isSingleDTDNode:options:error:") ~typ:(id @-> bool @-> ullong @-> ptr (id) @-> returning (id)) x isSingleDTDNode options error
