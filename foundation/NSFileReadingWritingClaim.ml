...
let _class_ = get_class "NSFileReadingWritingClaim"
...
let initWithPurposeID x ~readingURL ~options ~writingURL ~options_ ~claimer self = msg_send ~self ~cmd:(selector "initWithPurposeID:readingURL:options:writingURL:options:claimer:") ~typ:(id @-> id @-> ullong @-> id @-> ullong @-> ptr void @-> returning (id)) x readingURL options writingURL options_ claimer
