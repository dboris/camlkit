...
let assign ~x ~key ~value ~isNew self = msg_send ~self ~cmd:(selector "assign:key:value:isNew:") ~typ:(ullong @-> ptr (void) @-> ptr (void) @-> char @-> returning (void)) x key value isNew
let checkCount ~x self = msg_send ~self ~cmd:(selector "checkCount:") ~typ:(string @-> returning (void)) x
let containsKeys ~x ~values ~count self = msg_send ~self ~cmd:(selector "containsKeys:values:count:") ~typ:(ptr (ptr (void)) @-> ptr (ptr (void)) @-> ullong @-> returning (char)) x values count
