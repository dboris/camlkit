...
let _class_ = get_class "NSURLConnectionDelegateProxy"
...
let connection8 ~x ~didReceiveData ~lengthReceived self = msg_send ~self ~cmd:(selector "connection:didReceiveData:lengthReceived:") ~typ:(id @-> id @-> llong @-> returning (void)) x didReceiveData lengthReceived
...
let connection10 ~x ~didSendBodyData ~totalBytesWritten ~totalBytesExpectedToWrite self = msg_send ~self ~cmd:(selector "connection:didSendBodyData:totalBytesWritten:totalBytesExpectedToWrite:") ~typ:(id @-> llong @-> llong @-> llong @-> returning (void)) x didSendBodyData totalBytesWritten totalBytesExpectedToWrite
