...
let registerPort' ~x ~name ~nameServerPortNumber self = msg_send ~self ~cmd:(selector "registerPort:name:nameServerPortNumber:") ~typ:(id @-> id @-> ushort @-> returning (char)) x name nameServerPortNumber
