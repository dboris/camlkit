...
let isStandalone  self = msg_send ~self ~cmd:(selector "isStandalone") ~typ:(returning (char))
...
let setStandalone ~x self = msg_send ~self ~cmd:(selector "setStandalone:") ~typ:(char @-> returning (void)) x
...
let validateAndReturnError ~x self = msg_send ~self ~cmd:(selector "validateAndReturnError:") ~typ:(ptr (id) @-> returning (char)) x
