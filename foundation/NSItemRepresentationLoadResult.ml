...
let setWasOpenedInPlace ~x self = msg_send ~self ~cmd:(selector "setWasOpenedInPlace:") ~typ:(char @-> returning (void)) x
let urlWrapper  self = msg_send ~self ~cmd:(selector "urlWrapper") ~typ:(returning (id)) 
let wasOpenedInPlace  self = msg_send ~self ~cmd:(selector "wasOpenedInPlace") ~typ:(returning (char)) 