...
let encodeBool ~x ~forKey self = msg_send ~self ~cmd:(selector "encodeBool:forKey:") ~typ:(char @-> id @-> returning (void)) x forKey
...
let initRequiringSecureCoding ~x self = msg_send ~self ~cmd:(selector "initRequiringSecureCoding:") ~typ:(char @-> returning (id)) x
