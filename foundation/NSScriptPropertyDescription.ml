...
let initWithKey ~x ~type_ ~access ~isHidden ~accessGroupDescriptions self = msg_send ~self ~cmd:(selector "initWithKey:type:access:isHidden:accessGroupDescriptions:") ~typ:(id @-> id @-> ullong @-> char @-> id @-> returning (id)) x type_ access isHidden accessGroupDescriptions
