...
module Class = struct
  ...
  let bufferWithMaximumObjectCount ~x ~fullPolicy ~outputQueue self = msg_send ~self ~cmd:(selector "bufferWithMaximumObjectCount:fullPolicy:outputQueue:") ~typ:(ullong @-> llong @-> id @-> returning (id)) x fullPolicy outputQueue
