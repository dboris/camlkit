...
let insertObject1 ~x ~inAttributesAtIndex self = msg_send ~self ~cmd:(selector "insertObject:inAttributesAtIndex:") ~typ:(id @-> ullong @-> returning (void)) x inAttributesAtIndex
...
let normalizeAdjacentTextNodesPreservingCDATA ~x self = msg_send ~self ~cmd:(selector "normalizeAdjacentTextNodesPreservingCDATA:") ~typ:(char @-> returning (void)) x
...
let validateName ~x ~error self = msg_send ~self ~cmd:(selector "validateName:error:") ~typ:(ptr (id) @-> ptr (id) @-> returning (char)) x error