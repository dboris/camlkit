...
let isPartialStringValid ~x ~newEditingString ~errorDescription self = msg_send ~self ~cmd:(selector "isPartialStringValid:newEditingString:errorDescription:") ~typ:(id @-> ptr (id) @-> ptr (id) @-> returning (char)) x newEditingString errorDescription
