...
module Class = struct
  ...
  let localizedStringFromPersonNameComponents x ~style ~options self = msg_send ~self ~cmd:(selector "localizedStringFromPersonNameComponents:style:options:") ~typ:(id @-> llong @-> ullong @-> returning (id)) x style options
