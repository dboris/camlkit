...
let object_2 x ~withObservance ~willChangeValueForKeyOrKeys ~recurse ~forwardingValues self = msg_send ~self ~cmd:(selector "object:withObservance:willChangeValueForKeyOrKeys:recurse:forwardingValues:") ~typ:(id @-> id @-> id @-> bool @-> ptr void @-> returning (bool)) x withObservance willChangeValueForKeyOrKeys recurse forwardingValues