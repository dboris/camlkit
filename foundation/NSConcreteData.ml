...
let initWithBytes' x ~length ~copy ~freeWhenDone ~bytesAreVM self = msg_send ~self ~cmd:(selector "initWithBytes:length:copy:freeWhenDone:bytesAreVM:") ~typ:(ptr (void) @-> ullong @-> bool @-> bool @-> bool @-> returning (id)) x length copy freeWhenDone bytesAreVM
