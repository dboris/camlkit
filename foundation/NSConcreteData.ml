...
let initWithBytes' ~x ~length ~copy ~freeWhenDone ~bytesAreVM self = msg_send ~self ~cmd:(selector "initWithBytes:length:copy:freeWhenDone:bytesAreVM:") ~typ:(ptr (void) @-> ullong @-> char @-> char @-> char @-> returning (id)) x length copy freeWhenDone bytesAreVM
