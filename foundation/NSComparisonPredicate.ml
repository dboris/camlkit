...
module Class = struct
  ...
  let predicateWithLeftExpression' ~x ~rightExpression ~modifier ~type_ ~options self = msg_send ~self ~cmd:(selector "predicateWithLeftExpression:rightExpression:modifier:type:options:") ~typ:(id @-> id @-> ullong @-> ullong @-> ullong @-> returning (id)) x rightExpression modifier type_ options
