...
let initWithLeftExpression' ~x ~rightExpression ~modifier ~type_ ~options self = msg_send ~self ~cmd:(selector "initWithLeftExpression:rightExpression:modifier:type:options:") ~typ:(id @-> id @-> ullong @-> ullong @-> ullong @-> returning (id)) x rightExpression modifier type_ options
let initWithPredicateOperator2 ~x ~leftExpression ~rightExpression self = msg_send ~self ~cmd:(selector "initWithPredicateOperator:leftExpression:rightExpression:") ~typ:(id @-> id @-> id @-> returning (id)) x leftExpression rightExpression
...
let initWithPredicateOperator ~x ~leftKeyPath ~rightValue self = msg_send ~self ~cmd:(selector "initWithPredicateOperator:leftKeyPath:rightValue:") ~typ:(id @-> id @-> id @-> returning (id)) x leftKeyPath rightValue
