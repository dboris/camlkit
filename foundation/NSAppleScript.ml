...
let compileAndReturnError ~x self = msg_send ~self ~cmd:(selector "compileAndReturnError:") ~typ:(ptr (id) @-> returning (char)) x
...
let isCompiled  self = msg_send ~self ~cmd:(selector "isCompiled") ~typ:(returning (char)) 
