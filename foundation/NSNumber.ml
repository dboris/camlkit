...
let isEqualToNumber ~x self = msg_send ~self ~cmd:(selector "isEqualToNumber:") ~typ:(id @-> returning (char)) x
...
let matchesProcess ~x self = msg_send ~self ~cmd:(selector "matchesProcess:") ~typ:(id @-> returning (char)) x
