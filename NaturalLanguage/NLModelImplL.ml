...
(** Apple docs: {{:https://developer.apple.com/documentation/naturallanguage/nlmodelimpll?language=objc}NLModelImplL} *)

let self = get_class "NLModelImplL"
...
let initWithOwnedModelObject x ~configuration ~labelMap ~vocabularyMap ~documentFrequencyMap ~numberOfTrainingInstances self = msg_send ~self ~cmd:(selector "initWithOwnedModelObject:configuration:labelMap:vocabularyMap:documentFrequencyMap:numberOfTrainingInstances:") ~typ:((ptr void) @-> id @-> id @-> id @-> id @-> ullong @-> returning id) x configuration labelMap vocabularyMap documentFrequencyMap (ULLong.of_int numberOfTrainingInstances)
