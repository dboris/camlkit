...
let self = get_class "UIKeyboardEmojiCategoryBar"

let animateScrubberToRect x self = msg_send ~self ~cmd:(selector "animateScrubberToRect:") ~typ:(CGRect.t @-> returning void) x
let categorySelectedCircleRect x self = msg_send ~self ~cmd:(selector "categorySelectedCircleRect:") ~typ:(llong @-> returning CGRect.t) (LLong.of_int x)
