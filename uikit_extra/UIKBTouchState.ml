...
module C = struct
  ...
  let touchStateForTouchUUID x ~withTimestamp ~phase ~location ~pathIndex ~inView self = msg_send ~self ~cmd:(selector "touchStateForTouchUUID:withTimestamp:phase:location:pathIndex:inView:") ~typ:(id @-> double @-> llong @-> CGPoint.t @-> bool @-> id @-> returning (id)) x withTimestamp (LLong.of_int phase) location pathIndex inView
