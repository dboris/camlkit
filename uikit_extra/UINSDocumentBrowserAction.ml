...
let initWithIdentifier x ~localizedTitle ~availability ~handler self = msg_send ~self ~cmd:(selector "initWithIdentifier:localizedTitle:availability:handler:") ~typ:(id @-> id @-> llong @-> ptr void @-> returning (id)) x localizedTitle availability handler
