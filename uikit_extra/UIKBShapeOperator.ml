...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikbshapeoperator?language=objc}UIKBShapeOperator} *)

let self = get_class "UIKBShapeOperator"
...
let shapesByInsettingShapes x ~withInsets self = msg_send ~self ~cmd:(selector "shapesByInsettingShapes:withInsets:") ~typ:(id @-> UIEdgeInsets.t @-> returning id) x withInsets
