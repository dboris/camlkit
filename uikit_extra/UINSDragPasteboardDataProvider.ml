...
module C = struct
  let pasteboardWriterForItem x ~index ~handler self = msg_send ~self ~cmd:(selector "pasteboardWriterForItem:index:handler:") ~typ:(id @-> llong @-> id @-> returning (id)) x (LLong.of_int index) handler
end
...
let initWithItem x ~index ~handler ~mappedTypes self = msg_send ~self ~cmd:(selector "initWithItem:index:handler:mappedTypes:") ~typ:(id @-> llong @-> id @-> id @-> returning (id)) x (LLong.of_int index) handler mappedTypes
