...
let destroyScenesWithPersistentIdentifiers x ~animationType ~destroySessions ~completion self = msg_send ~self ~cmd:(selector "destroyScenesWithPersistentIdentifiers:animationType:destroySessions:completion:") ~typ:(id @-> ullong @-> bool @-> ptr void @-> returning (void)) x animationType destroySessions completion
