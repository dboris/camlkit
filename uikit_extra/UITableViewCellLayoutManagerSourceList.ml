...
let self = get_class "UITableViewCellLayoutManagerSourceList"
...
let textRectForCell x ~rowWidth ~forSizing self = msg_send ~self ~cmd:(selector "textRectForCell:rowWidth:forSizing:") ~typ:(id @-> double @-> bool @-> returning CGRect.t) x rowWidth forSizing