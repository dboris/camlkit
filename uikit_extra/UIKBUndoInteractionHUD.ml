...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikbundointeractionhud?language=objc}UIKBUndoInteractionHUD} *)

let self = get_class "UIKBUndoInteractionHUD"
...
let controlForType x self = msg_send ~self ~cmd:(selector "controlForType:") ~typ:(llong @-> returning id) (LLong.of_int x)
let createSeparatorView self = msg_send ~self ~cmd:(selector "createSeparatorView") ~typ:(returning id)
let initWithKeyboardAppearance x ~isRTL ~mode ~sceneBounds self = msg_send ~self ~cmd:(selector "initWithKeyboardAppearance:isRTL:mode:sceneBounds:") ~typ:(llong @-> bool @-> llong @-> CGRect.t @-> returning id) (LLong.of_int x) isRTL (LLong.of_int mode) sceneBounds
...
let updateControlWithDirection x ~travelProgress ~isRTL self = msg_send ~self ~cmd:(selector "updateControlWithDirection:travelProgress:isRTL:") ~typ:(llong @-> double @-> bool @-> returning void) (LLong.of_int x) travelProgress isRTL
