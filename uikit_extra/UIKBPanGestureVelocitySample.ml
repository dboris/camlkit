...
let _class_ = get_class "UIKBPanGestureVelocitySample"

let dt self = msg_send ~self ~cmd:(selector "dt") ~typ:(returning (double))
let end_ self = msg_send ~self ~cmd:(selector "end") ~typ:(returning (CGPoint.t))
...
let start self = msg_send ~self ~cmd:(selector "start") ~typ:(returning (CGPoint.t))