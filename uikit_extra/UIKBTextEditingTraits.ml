...
module Class = struct
  let traitsForEditingInteractionWithFirstResponder x ~canRespondBlock ~keyMaskFlags self = msg_send ~self ~cmd:(selector "traitsForEditingInteractionWithFirstResponder:canRespondBlock:keyMaskFlags:") ~typ:(id @-> ptr void @-> ullong @-> returning (id)) x canRespondBlock keyMaskFlags
  let traitsWithResponder x ~keyMaskFlags self = msg_send ~self ~cmd:(selector "traitsWithResponder:keyMaskFlags:") ~typ:(id @-> ullong @-> returning (id)) x keyMaskFlags
end
...
let initWithResponder x ~canRespondBlock ~keyMaskFlags ~sender self = msg_send ~self ~cmd:(selector "initWithResponder:canRespondBlock:keyMaskFlags:sender:") ~typ:(id @-> ptr void @-> ullong @-> id @-> returning (id)) x canRespondBlock keyMaskFlags sender
