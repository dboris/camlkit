...
(** Apple docs: {{:https://developer.apple.com/documentation/uikit/uikeyboardemojiwellview?language=objc}UIKeyboardEmojiWellView} *)

let self = get_class "UIKeyboardEmojiWellView"
...
let compositeImageSize self = msg_send ~self ~cmd:(selector "compositeImageSize") ~typ:(returning CGSize.t)
let fontUsingSilhouette x ~size self = msg_send ~self ~cmd:(selector "fontUsingSilhouette:size:") ~typ:(ullong @-> double @-> returning id) (ULLong.of_int x) size
...
let setStringRepresentation' x ~silhouette self = msg_send ~self ~cmd:(selector "setStringRepresentation:silhouette:") ~typ:(id @-> ullong @-> returning void) x (ULLong.of_int silhouette)
