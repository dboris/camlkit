...
let computeOffsetForOffset x ~withSize ~chromeBuffer ~onScreenSize self = msg_send ~self ~cmd:(selector "computeOffsetForOffset:withSize:chromeBuffer:onScreenSize:") ~typ:(CGPoint.t @-> CGSize.t @-> UIEdgeInsets.t @-> CGSize.t @-> returning CGPoint.t) x withSize chromeBuffer onScreenSize
