...
let _class_ = get_class "UIPDFUnderlineAnnotation"
...
let drawLayerRotated x ~inContext ~selection ~rectangles self = msg_send ~self ~cmd:(selector "drawLayerRotated:inContext:selection:rectangles:") ~typ:(id @-> id @-> id @-> ullong @-> returning (void)) x inContext selection rectangles
let drawLayerUpright x ~inContext ~selection ~rectangles self = msg_send ~self ~cmd:(selector "drawLayerUpright:inContext:selection:rectangles:") ~typ:(id @-> id @-> id @-> ullong @-> returning (void)) x inContext selection rectangles
