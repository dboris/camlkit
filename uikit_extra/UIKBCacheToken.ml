...
module C = struct
  ...
  let tokenForKeyMask x ~style ~displayInsets self = msg_send ~self ~cmd:(selector "tokenForKeyMask:style:displayInsets:") ~typ:(id @-> ptr void @-> ptr void @-> returning (id)) x style displayInsets
