...
let initWithSlotDrawer x ~options self = msg_send ~self ~cmd:(selector "initWithSlotDrawer:options:") ~typ:(id @-> ullong @-> returning (id)) x (ULLong.of_int options)
let remoteContentForLayerContextWithId x ~style ~tag self = msg_send ~self ~cmd:(selector "remoteContentForLayerContextWithId:style:tag:") ~typ:(ullong @-> id @-> id @-> returning (id)) (ULLong.of_int x) style tag
