...
let importDocumentAtURL x ~nextToDocumentAtURL ~mode ~completionHandler self = msg_send ~self ~cmd:(selector "importDocumentAtURL:nextToDocumentAtURL:mode:completionHandler:") ~typ:(id @-> id @-> ullong @-> ptr void @-> returning (void)) x nextToDocumentAtURL (ULLong.of_int mode) completionHandler
