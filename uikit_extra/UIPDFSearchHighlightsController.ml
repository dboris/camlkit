...
let indexOfColumnBreakStartingAt x self = msg_send ~self ~cmd:(selector "indexOfColumnBreakStartingAt:") ~typ:(ullong @-> returning (ullong)) x
...
let makeType2Path x ~to_ ~shadowPath self = msg_send ~self ~cmd:(selector "makeType2Path:to:shadowPath:") ~typ:(ptr void @-> ullong @-> ptr void @-> returning (void)) x to_ shadowPath
...
let makeType3Path x ~from ~to_ ~shadowPath self = msg_send ~self ~cmd:(selector "makeType3Path:from:to:shadowPath:") ~typ:(ptr void @-> ullong @-> ullong @-> ptr void @-> returning (void)) x from to_ shadowPath
let makeType4Path x ~from ~to_ ~shadowPath self = msg_send ~self ~cmd:(selector "makeType4Path:from:to:shadowPath:") ~typ:(ptr void @-> ullong @-> ullong @-> ptr void @-> returning (void)) x from to_ shadowPath
...
let unionFrom x ~to_ self = msg_send_stret ~self ~cmd:(selector "unionFrom:to:") ~typ:(ullong @-> ullong @-> returning (CGRect.t)) ~return_type:CGRect.t x to_