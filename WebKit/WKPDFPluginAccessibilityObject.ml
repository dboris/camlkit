...
let convertRectToScreenSpace x self = msg_send ~self ~cmd:(selector "convertRectToScreenSpace:") ~typ:(CGRect.t @-> returning CGRect.t) x
...
let pluginElement self = msg_send ~self ~cmd:(selector "pluginElement") ~typ:(returning id)
...
let setPluginElement x self = msg_send ~self ~cmd:(selector "setPluginElement:") ~typ:(id @-> returning void) x