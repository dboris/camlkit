...
let initWithNetworkSession x ~wrapper ~withCredentials self = msg_send ~self ~cmd:(selector "initWithNetworkSession:wrapper:withCredentials:") ~typ:(id @-> (ptr void) @-> bool @-> returning id) x wrapper withCredentials
