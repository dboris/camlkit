...
let initWithLegacyCustomProtocolManagerProxy x ~customProtocolID ~request self = msg_send ~self ~cmd:(selector "initWithLegacyCustomProtocolManagerProxy:customProtocolID:request:") ~typ:((ptr void) @-> id @-> id @-> returning id) x customProtocolID request