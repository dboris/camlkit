...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nstoolbartitleview?language=objc}NSToolbarTitleView} *)

let self = get_class "NSToolbarTitleView"
...
let expandedOverlaySize self = msg_send ~self ~cmd:(selector "expandedOverlaySize") ~typ:(returning CGSize.t)
let expandedTitleOnlyOverlaySize self = msg_send ~self ~cmd:(selector "expandedTitleOnlyOverlaySize") ~typ:(returning CGSize.t)
...
let proxyAcceleratorExclusionRect self = msg_send ~self ~cmd:(selector "proxyAcceleratorExclusionRect") ~typ:(returning CGRect.t)
