...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nstitlebarview?language=objc}NSTitlebarView} *)

let self = get_class "NSTitlebarView"
...
let sectionAtIndex x ~didChangeDividerExtentRect self = msg_send ~self ~cmd:(selector "sectionAtIndex:didChangeDividerExtentRect:") ~typ:(ullong @-> CGRect.t @-> returning void) (ULLong.of_int x) didChangeDividerExtentRect
...
let updateMaterialForSection x ~atIndex self = msg_send ~self ~cmd:(selector "updateMaterialForSection:atIndex:") ~typ:(id @-> ullong @-> returning void) x (ULLong.of_int atIndex)
