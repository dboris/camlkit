...
let self = get_class "NSTableViewActionButton"
...
let layoutStateForBounds x self = msg_send ~self ~cmd:(selector "layoutStateForBounds:") ~typ:(CGRect.t @-> returning void) x
let maskedCornersMask self = msg_send ~self ~cmd:(selector "maskedCornersMask") ~typ:(returning ullong)
let mouseDown x self = msg_send ~self ~cmd:(selector "mouseDown:") ~typ:(id @-> returning void) x
let requiredSize self = msg_send_stret ~self ~cmd:(selector "requiredSize") ~typ:(returning CGSize.t) ~return_type:CGSize.t
