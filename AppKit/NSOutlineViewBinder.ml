...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsoutlineviewbinder?language=objc}NSOutlineViewBinder} *)

let self = get_class "NSOutlineViewBinder"
...
let modifyObservingOutlineViewChildrenOfItem x ~withOption self = msg_send ~self ~cmd:(selector "modifyObservingOutlineViewChildrenOfItem:withOption:") ~typ:(id @-> ullong @-> returning void) x (ULLong.of_int withOption)
...
let outlineView4 x ~child ~ofItem self = msg_send ~self ~cmd:(selector "outlineView:child:ofItem:") ~typ:(id @-> llong @-> id @-> returning id) x (LLong.of_int child) ofItem
...
let outlineView6 x ~willDisplayCell ~forTableColumn ~row self = msg_send ~self ~cmd:(selector "outlineView:willDisplayCell:forTableColumn:row:") ~typ:(id @-> id @-> id @-> llong @-> returning void) x willDisplayCell forTableColumn (LLong.of_int row)
let outlineView7 x ~willDisplayOutlineCell ~forTableColumn ~row self = msg_send ~self ~cmd:(selector "outlineView:willDisplayOutlineCell:forTableColumn:row:") ~typ:(id @-> id @-> id @-> llong @-> returning void) x willDisplayOutlineCell forTableColumn (LLong.of_int row)
