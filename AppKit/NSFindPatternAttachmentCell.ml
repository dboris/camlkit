...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsfindpatternattachmentcell?language=objc}NSFindPatternAttachmentCell} *)

let self = get_class "NSFindPatternAttachmentCell"
...
let pullDownRectForBounds x self = msg_send ~self ~cmd:(selector "pullDownRectForBounds:") ~typ:(CGRect.t @-> returning CGRect.t) x
