...
let self = get_class "NSToolbarConfigPanel"
...
let collectionView8 x ~draggingSession ~endedAtPoint ~dragOperation self = msg_send ~self ~cmd:(selector "collectionView:draggingSession:endedAtPoint:dragOperation:") ~typ:(id @-> id @-> CGPoint.t @-> ullong @-> returning void) x draggingSession endedAtPoint (ULLong.of_int dragOperation)
