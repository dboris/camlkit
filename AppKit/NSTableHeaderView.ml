...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nstableheaderview?language=objc}NSTableHeaderView} *)

let self = get_class "NSTableHeaderView"
...
let headerRectOfColumn x self = msg_send_stret ~self ~cmd:(selector "headerRectOfColumn:") ~typ:(llong @-> returning CGRect.t) ~return_type:CGRect.t (LLong.of_int x)
