...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nspopupbuttoncell?language=objc}NSPopUpButtonCell} *)

let self = get_class "NSPopUpButtonCell"
...
let drawTitle x ~withFrame ~inView self = msg_send_stret ~self ~cmd:(selector "drawTitle:withFrame:inView:") ~typ:(id @-> CGRect.t @-> id @-> returning CGRect.t) ~return_type:CGRect.t x withFrame inView
...
let keyEquivalentRectForBounds x self = msg_send_stret ~self ~cmd:(selector "keyEquivalentRectForBounds:") ~typ:(CGRect.t @-> returning CGRect.t) ~return_type:CGRect.t x
...
let setArrowPosition x self = msg_send ~self ~cmd:(selector "setArrowPosition:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
