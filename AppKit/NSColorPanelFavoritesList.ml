...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nscolorpanelfavoriteslist?language=objc}NSColorPanelFavoritesList} *)

let self = get_class "NSColorPanelFavoritesList"
...
let collectionView4 x ~acceptDrop ~indexPath ~dropOperation self = msg_send ~self ~cmd:(selector "collectionView:acceptDrop:indexPath:dropOperation:") ~typ:(id @-> id @-> id @-> llong @-> returning bool) x acceptDrop indexPath (LLong.of_int dropOperation)
let collectionView5 x ~draggingSession ~endedAtPoint ~dragOperation self = msg_send ~self ~cmd:(selector "collectionView:draggingSession:endedAtPoint:dragOperation:") ~typ:(id @-> id @-> CGPoint.t @-> ullong @-> returning void) x draggingSession endedAtPoint (ULLong.of_int dragOperation)
...
let storeIndexForColorIndex x self = msg_send ~self ~cmd:(selector "storeIndexForColorIndex:") ~typ:(llong @-> returning llong) (LLong.of_int x)
