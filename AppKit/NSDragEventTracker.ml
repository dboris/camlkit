...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsdrageventtracker?language=objc}NSDragEventTracker} *)

let self = get_class "NSDragEventTracker"
...
let setDiscardEventMask x self = msg_send ~self ~cmd:(selector "setDiscardEventMask:") ~typ:(ullong @-> returning void) (ULLong.of_int x)
...
let setStage x self = msg_send ~self ~cmd:(selector "setStage:") ~typ:(llong @-> returning void) (LLong.of_int x)
