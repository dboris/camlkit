...
let self = get_class "NSToolTipManager"
...
let onScreenToolTipWindowFrameOriginForToolTip x ~windowFrame ~where ~location self = msg_send ~self ~cmd:(selector "onScreenToolTipWindowFrameOriginForToolTip:windowFrame:where:location:") ~typ:(id @-> CGRect.t @-> CGPoint.t @-> CGPoint.t @-> returning CGPoint.t) x windowFrame where location
...
let toolTipContentMargin self = msg_send ~self ~cmd:(selector "toolTipContentMargin") ~typ:(returning CGSize.t)
