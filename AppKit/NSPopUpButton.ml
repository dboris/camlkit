...
let setUINSMenu x self = msg_send ~self ~cmd:(selector "setUINSMenu:") ~typ:(id @-> returning void) x
let sizeToFit self = msg_send ~self ~cmd:(selector "sizeToFit") ~typ:(returning void)
let sizingBehavior self = msg_send ~self ~cmd:(selector "sizingBehavior") ~typ:(returning llong) |> LLong.to_int
