...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsoutlineview?language=objc}NSOutlineView} *)

let self = get_class "NSOutlineView"
...
let frameOfCellAtColumn x ~row self = msg_send ~self ~cmd:(selector "frameOfCellAtColumn:row:") ~typ:(llong @-> llong @-> returning CGRect.t) (LLong.of_int x) (LLong.of_int row)
let frameOfOutlineCellAtRow x self = msg_send ~self ~cmd:(selector "frameOfOutlineCellAtRow:") ~typ:(llong @-> returning CGRect.t) (LLong.of_int x)
