...
let languageLevel self = msg_send ~self ~cmd:(selector "languageLevel") ~typ:(returning llong) |> LLong.to_int
...
let statusForTable x self = msg_send ~self ~cmd:(selector "statusForTable:") ~typ:(id @-> returning ullong) x |> ULLong.to_int
