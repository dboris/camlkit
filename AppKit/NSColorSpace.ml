...
let colorSpaceModel self = msg_send ~self ~cmd:(selector "colorSpaceModel") ~typ:(returning llong) |> LLong.to_int
...
let numberOfColorComponents self = msg_send ~self ~cmd:(selector "numberOfColorComponents") ~typ:(returning llong) |> LLong.to_int