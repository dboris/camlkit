...
let rectOfPathComponentCell x ~withFrame ~inView self = msg_send_stret ~self ~cmd:(selector "rectOfPathComponentCell:withFrame:inView:") ~typ:(id @-> CGRect.t @-> id @-> returning CGRect.t) ~return_type:CGRect.t x withFrame inView
let removePathComponentCellAtIndex x self = msg_send ~self ~cmd:(selector "removePathComponentCellAtIndex:") ~typ:(llong @-> returning void) (LLong.of_int x)
