...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nslabelviewcell?language=objc}NSLabelViewCell} *)

let self = get_class "NSLabelViewCell"
...
let part x ~boundsWithFrame self = msg_send ~self ~cmd:(selector "part:boundsWithFrame:") ~typ:(llong @-> CGRect.t @-> returning CGRect.t) (LLong.of_int x) boundsWithFrame
