...
(** Apple docs: {{:https://developer.apple.com/documentation/appkit/nsservicesrolloverview?language=objc}NSServicesRolloverView} *)

let self = get_class "NSServicesRolloverView"

let dealloc self = msg_send ~self ~cmd:(selector "dealloc") ~typ:(returning void)
let delegate self = msg_send ~self ~cmd:(selector "delegate") ~typ:(returning id)
let hitTest x self = msg_send ~self ~cmd:(selector "hitTest:") ~typ:(CGPoint.t @-> returning id) x
let initWithDelegate x ~style self = msg_send ~self ~cmd:(selector "initWithDelegate:style:") ~typ:(id @-> llong @-> returning id) x (LLong.of_int style)
